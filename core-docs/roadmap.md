...
### Phase 23.C follow-ups — Deferred review items *(parked from PR #92, PR #94, and PR (#96 TBD) staff reviews — captured here so they don't rot in closed PR bodies)*

The expand-to-payload pattern landed (PR #92), the layout-stability + error-state polish landed (PR #94), and the discoverability + retry polish landed (PR #96 TBD). Two items remain that need an architectural decision or a separate workstream:

- ~~**23.C.f1 — Discoverability affordance on `.tool-line__head`.**~~ ✅ Shipped in PR #96 TBD. The `·` dot is replaced by a `ChevronRight` (lucide-react, size 12, strokeWidth 1.5) that rotates 90° when `aria-expanded="true"`. Same monochrome weight at rest as the dot; carries the disclosure semantics without ambiguity.

- **23.C.f2 — Honor `BlockProps.expanded` / `onToggleExpanded`.** ToolUseLine ignores the BlockProps contract and tracks expand state locally. Engineer reviewer (PR #92 round 1) noted this means the parent `WorkspaceThread` can't collapse all expanded rows on tab switch (or any other parent-driven sweep). The original deliverable explicitly said "disclosed state persists per-mount only," so the local state is correct for v1, but if/when a parent needs to drive collapse (e.g., focus-mode, "collapse all"), wiring through is the migration path. Wiring would also need to extend the parent's payload state to track loading / error / loaded phases (today the parent only stores resolved `PayloadRef`s), since ToolUseLine owns those phase transitions. Touches `WorkspaceThread.tsx` (state map + props), the `BlockProps` contract, and a synced phase model. Owner: whichever workstream first needs parent-driven collapse — speculative until then. ~1 day frontend.

- **23.C.f3 — Coalesce consecutive same-tool rows.** Already listed under "Out of scope (v1)" above; restated here so the Phase 23.C follow-up trail is one place. v2 polish (e.g., "Read 4 files" disclosure expanding to four citations), needs a coalescing primitive in the parent thread (not a block-side change) that respects per-call expand state. Owner: future Phase 23 polish PR. ~1.5 days frontend.

- ~~**23.C.f4 — Distinguish transient from permanent fetch failures.**~~ ✅ Shipped in PR #96 TBD via a frontend-only path: the error state renders a "Try again" button that clears `fetchedRef`/`inflightRef` and refires `fetchPayload`. Sidesteps the Rust-side error classification by letting the user choose. A permanent 404 still costs one extra IPC on the retry click (which then re-renders the same error state); transient failures recover with one click. The Rust-side classification (404 → permanent → no retry button; 5xx / IPC error → transient → retry button) remains a future polish if the extra IPC for permanent failures becomes a problem in dogfood.
...
## Phase 24 — Chat pass-through (Build, ✅ complete 2026-05-13)
...
> **Sequencing note (2026-05-13):** This phase now lands *before* Phase 24H, not after. Original order was 24 → 24H → 24I → 25; reordered to 24 → 24I → 24H → 25. Reasons: 24H's render-altitude FOLLOW-UPs want an integration harness underneath them (writing them first means re-doing them later at the right altitude); PR #125's deterministic `read_all` rowid-tiebreaker test was already filed *into* 24I; the coalescer-cleanup pre-condition ("≥1 dogfood week with no `show_chat_v2: false` overrides") elapses naturally while 24I is in flight, letting 24H ship the cleanup confidently; 24I is smaller (~3–5 days vs. ~1 week), lower-risk to land first while chat-v2 is still settling from the flag flip. Mitigates the cognitive-load overlap that PR #122 staff review flagged.
...
- **Deterministic test for `read_all` rowid tiebreaker** (filed from PR #125, originally docked under 24H). PR #125 fixed a 1/10 CI flake in `projector_apply_is_idempotent_per_sequence` by changing `read_all`'s ORDER BY from `(timestamp ASC, sequence ASC)` to `(timestamp ASC, rowid ASC)` — `sequence` is per-stream so timestamp ties interleaved events non-causally and broke the projector's dedup. The fix is empirically stable (30/30 post-fix), but stability is absence of evidence, not a contract. A future refactor back to `sequence ASC` would only resurface the flake probabilistically. Add a deterministic test that forces two events into the same RFC3339 instant (via a `#[doc(hidden)] pub fn force_timestamp_for_test()` helper on `SqliteEventStore` that UPDATEs the timestamp column post-insert) and asserts `read_all` returns them in insertion order regardless. Lands here since both want test-only DB helpers.
...
> **Sequencing note (2026-05-13):** Now lands *after* Phase 24I (see 24I sequencing note above). Render-altitude FOLLOW-UPs below are written against the 24I harness, not as bespoke render-only mounts.
...
- **First-run audit.** Walk the onboarding flow from PR #24 against any subtractions in the Phase-24H release (stub renderers, hidden detectors, model selector behind flag). Update onboarding before the cull lands.
- **In-app "What's new" card for the Phase-24H release.** One-time, dismissible, auto-shown on first launch. Copy is manager-voiced — strategic narrowing, not defensive listing. Draft: *"We've focused the cockpit on what's essential. The model selector and additional analysis signals will return as they're polished. Less surface, more trust."* Final copy proofread before Phase 24H ships.
...
- **Demo gatekeeping.** Record golden-path screencast: open Designer → create project → start workspace → post message → see streamed response → see artifact → approve a tool use → ship a track. Save to `core-docs/screencasts/` keyed to the Phase-24H release tag. (Playwright binding lands in 26H; the screencast convention starts now.)
- **Visual baselines for the Phase 24 chat surface.** PR #120 added `ChatStreamRenderer`, `ToolUseBlock`, `ThinkingBlock`, `InterruptedMarker`, `LegacyChatBanner`, `LoadingState` behind `show_chat_v2` (default OFF at the time). PR #134 flipped the flag default ON. Regenerate visual-regression baselines with the flag forced on (test fixtures override `flagsStore`); confirm dark-mode + smallest-window (`960×640`) variants. (Filed from PR #120 staff-review pre-merge spot-check.)
- **`bootReplaying` LoadingState→EmptyState transition test.** PR #120's pre-merge review fixed an empty-state-during-fetch UX BLOCKER by adding the `bootReplaying` flag. A defaults test locks the contract (`bootReplaying: true` at store creation), but no render-level test exercises the LoadingState→EmptyState transition through `ChatStreamRenderer`. Write against the 24I harness now that `show_chat_v2` is ON by default. (Filed from PR #120 staff-review pre-merge spot-check.)
- **WorkspaceThread queue auto-dispatch render-altitude test.** PR #124's `compose-queue.test.tsx` covers the ComposeDock surface end-to-end and the store-level invariants the auto-dispatch effect relies on, but the `WorkspaceThread.tsx` effect itself (working→idle transition fires `onSend` with the queued text and clears the queue) lacks a render-altitude test. Mount `WorkspaceThread` against a mock IPC client; queue a message via `setQueuedMessage`; flip the activity slice from `working` → `idle`; assert `onSend` was called with the queued body and the chip removed. Pairs naturally with the `bootReplaying` test above — both gate on `show_chat_v2` paths becoming testable. (Filed from PR #124 staff-review.)
- **Render-altitude test for `InterruptAnnouncement` aria-live region.** PR #125 added the assertive announcement on `AgentTurnEnded { stop_reason: Interrupted }` per spec §5.7. The contract is mechanical (announced turn-ids tracked in a ref so each interruption announces exactly once); written against the 24I harness with `show_chat_v2: true` + a primed `chatThread` state. Pairs naturally with the `bootReplaying` test above and the queue auto-dispatch render-altitude test below — same fixture skeleton. (Filed from PR #125 staff-review.)
- **Render-altitude tests for §5.6 markers + `ErrorAnnouncement`.** Step 12 (this PR) added `ErrorMarker` (`stop_reason: error`) + `MaxTokensMarker` (`stop_reason: max_tokens`) inline siblings to `InterruptedMarker`, plus the assertive `ErrorAnnouncement` aria-live region in `WorkspaceThread` carrying the §5.6 row-2 copy verbatim. Same render-altitude gate as `InterruptAnnouncement` above — all four pieces (interrupted / error / max-tokens / error-announcement) share the same `WorkspaceThread` + chat-v2 fixture skeleton and should be tested together. Cover: marker mounts under the right turn, copy matches §5.6 verbatim, `ErrorAnnouncement` fires exactly once per errored turn (not per re-render), `MaxTokensMarker` doesn't announce (spec §5.7 row 274 doesn't list max-tokens as a live event). (Filed from Step 12 PR.)
- **Approval-expired surface (spec §5.6 row 6).** Today `InboxPermissionHandler` emits `ApprovalDenied { reason: "timeout" }` after the 5-min approval window, but no renderer surfaces the spec copy ("Approval expired. The agent stopped waiting; send your message again to continue."). The approval artifact silently disappears from the inbox. To wire: a new consumer in `WorkspaceThread` (or a sibling component) watches for `ApprovalDenied { reason }` events on the workspace stream, and when `reason === "timeout"` renders an inline marker in the chat thread (same register as `ErrorMarker` — `--color-foreground-muted` + `--color-warning` pill). Filed as a Phase 24H follow-up rather than expanded into Step 12 because it requires a new event-renderer wiring + inline marker placement decision + replay-safety considerations (legacy ApprovalDenied events shouldn't suddenly start surfacing markers) — separate cycle of design + implementation. (Filed from Step 12 PR.)
- **"Restarting…" transient copy (spec §5.6 row 1, first leg).** Spec text: *"Claude stopped unexpectedly. Restarting…" (auto-restart on next send; if restart also fails: "Couldn't reach Claude...")*. Step 12 (this PR) aligned the second-fail copy in `humanize_dispatch_error` to spec verbatim, but the *first* leg — the transient "Restarting…" message while the auto-respawn is in flight — is currently silent: the respawn happens within `post_message`'s catch-and-retry, and the user only sees the final outcome. Surfacing the transient would require a new IPC state (`PostMessageProgress { phase: "retrying" }` or similar) so the frontend can paint the banner. Filed because the state machine extension warrants a separate cycle. (Filed from Step 12 PR.)
- **Coalescer + chat-v1-specific arms cleanup (step 3 spec cleanup).** Phase 24 step 13 (this PR) flipped `show_chat_v2` default ON. The `spawn_message_coalescer` function in `apps/desktop/src-tauri/src/core_agents.rs` stays load-bearing as the broadcast→store bridge — its `AgentTurnStarted` / `AgentContentBlockStarted` / `AgentContentBlockDelta` / `AgentContentBlockEnded` / `AgentToolResult` / `AgentTurnEnded` / `CostRecorded` arms persist chat-v2 events. The chat-v1-specific arms (`MessagePosted` with the 120 ms coalescing timer, `ArtifactProduced { kind: Report }`, the `PendingMessage` struct with `first_seen_at`, the `last_user_tab_by_workspace` map, the legacy coalescer tests `coalescer_drops_user_echoes` / `coalescer_separates_keys` / `coalescer_attributes_reply_to_tab_at_first_recv_not_at_flush` / `coalescer_flushed_artifact_uses_first_chunk_timestamp` / `coalescer_first_seen_resets_between_bursts` / `coalescer_flushed_artifact_predates_subsequent_user_post`) become dead code once dogfood confirms `show_chat_v2: true` is stable. Estimated impact: ~600 LOC removable in core_agents.rs + companion getter/setter pairs on `AppCore`. Cleanup pre-condition: ≥1 dogfood week with no `show_chat_v2: false` overrides in user settings.json + no critical chat-friction reports. (Filed from Step 13 PR.)
- **Wire `show_compose_stop_and_send` kill-switch flag.** PR #124 introduced the queue + stop-and-send modes per spec §5.4 but reserved the flag as a kill-switch *only* — defaulting ON without a runtime gate. If the alternate path (interrupt + queue) shows real issues during dogfood (interrupt round-trip latency, race with AgentTurnEnded, etc.), the gate needs to exist before `show_chat_v2` flips ON broadly. Wire the flag before the broad flag-flip: read in ComposeDock to suppress the ⌘⏎ chord + the SendMenu second row when off; default ON. (Filed from PR #124 implementation deferral.)
- **Resolve `--motion-pulse` 1.2s vs 1.6s discrepancy.** Phase 24 §5.2 says the activity pulse "uses a token: `--motion-pulse` (defined as 1.6s ease-in-out)." The token in `packages/ui/styles/tokens.css` is `1200ms`. The Step 10 PR did not change the global token because the same token drives the tab-strip activity badge — re-pacing it without spec coverage for the sibling surface would be silent drift. Two options for resolution: (a) bump the token to `1600ms` globally and update the spec/tab-badge note to confirm both surfaces should pulse in tandem; (b) keep `1200ms` and update spec §5.2 to match the shipped value. The decision should fall under the Phase 24H Mini polish pass with one shared resolution across both surfaces. (Filed from Step 10 PR.)

### Chat-UX research synthesis follow-ups (filed 2026-05-13)

Five items filed from the 2026-05-13 chat-UX research pass (research report + parallel staff-engineer + staff-UX reviews; full synthesis in `phase-24-pass-through-chat.md` Appendix C). Each is friction closure, test coverage, or design-language enforcement per ADR 0009 §1 — no new feature tracks. Other research items routed to parking lot or dropped; see Appendix C for the full triage.

- **Scroll-stickiness on the Phase 24 chat surface.** Closes spec Q4 ("Scroll-anchor behavior when a 5000-token response streams in"). The legacy artifact-list path (`WorkspaceThread.tsx:597–649`) has `stickRef` + `onThreadScroll` + `useLayoutEffect` autoscroll + Jump-to-latest pill (32 px near-bottom heuristic). The Phase 24 chat surface (`WorkspaceThread.tsx:663–674`, wrapping `ChatStreamRenderer`) has none of this — a regression introduced when `show_chat_v2` flipped default ON in PR #134. Port the legacy pattern (32 px threshold matches what users already learned; do not invent 96 px) and add a `ResizeObserver` so streaming text height changes don't fight the scroll loop. Programmatic-scroll guard required to prevent auto-pin scrolls from re-firing `onScroll` and flipping `stickRef` false (the legacy path got away without it because writes happen in `useLayoutEffect`, but the chat-v2 surface streams via store-subscription updates that may not share the same flush boundary). Jump-to-latest pill reuses `.thread__jump-latest` class verbatim. Render-altitude test against the 24I harness: stream a 500-token response while scrolled up, assert the thread stays put + pill appears; click pill, assert re-pin. (Filed from chat-UX research synthesis, 2026-05-13.)

- **Visible Stop button while a turn is open.** Today the interrupt mechanism (PR #125) is reachable via Esc-when-composer-focused and ⌘⏎ (stop-and-send). Mouse-only users with the composer unfocused have no path to interrupt without sending a message. Add a small `IconButton` (square / stop glyph) mounted as a sibling of the Send button in `ComposeDock`'s footer (not a swap — keep both, so layout doesn't reflow on turn boundaries). Visible only while `isSubprocessRunning` for the focused tab; reuses the existing `ipcClient().interruptTurn(workspaceId, tabId)` IPC — no backend changes. Tooltip carries the shortcut hint "Esc". Microcopy: "Stop" (verb), not "Cancel" (ambiguous with the queue). aria-label="Stop agent". Friction closure for trackpad-only dogfood users. (Filed from chat-UX research synthesis, 2026-05-13.)

- **Streaming caret on the open text block.** `TextBlock` (`ChatStreamRenderer.tsx:193–221`) renders raw delta as `pre-wrap` while open with no trailing indicator — the surface can look frozen mid-stream when deltas pause for thinking or web search (the activity dot is in the composer, not in-line with the text). Add a 2 px-wide pseudo-element on `.block__message-body--streaming::after` using `currentColor`; blink keyframe gated on `prefers-reduced-motion: reduce` (caret stays steady, not blinking, when reduced-motion is set). Reuses the `--motion-pulse` rhythm — bundle the resolution with the existing 1.2 s vs 1.6 s reconciliation item above so the caret blink and activity-dot pulse share one cadence. `aria-hidden` (the streaming text container itself is the live region). Implements Mini Craft Principle 1 ("cursor alone for streaming text"). Estimated ~20 LOC + one CSS keyframe. (Filed from chat-UX research synthesis, 2026-05-13.)

- **Reduced-motion audit on tool-line + thinking chevrons.** Spec §5.8 UX-3 promises "tool-call expand/collapse and thinking-block disclosure honor `prefers-reduced-motion`: instant disclosure, no rotate/transform." The shipped CSS for `.thinking__chevron` and `.tool-line__chevron` (transitions defined in `ChatStreamRenderer.tsx`'s sibling CSS) has not been verified against the prefers-reduced-motion media query in 24's shipped state. Run `audit-a11y` against `ChatStreamRenderer.tsx` + its CSS; verify chevron rotation collapses to instant under `prefers-reduced-motion: reduce`; also verify the thinking-disclosure body slide-in is gated. Pair with the visual-baselines regen item above — same CSS files touched. (Filed from chat-UX research synthesis, 2026-05-13; design-language enforcement per ADR 0009 §1 category 3.)

- **Regression test: queue does not flush on intermediate `AgentContentBlockEnded`.** The queue auto-dispatch contract is "fire on `AgentTurnEnded`, any stop_reason." A future refactor of the dispatch effect could accidentally shortcut on per-block `ended` events — which is exactly the Claude Code bug at [anthropics/claude-code#49373](https://github.com/anthropics/claude-code/issues/49373) where their own queue flushes on intermediate LLM pauses and derails work. Lock our behavior with a deterministic fixture against the 24I harness: emit `AgentTurnStarted` → `AgentContentBlockStarted{Text}` → `AgentContentBlockEnded` → `AgentContentBlockStarted{ToolUse}` → assert queued message has NOT dispatched. Then emit `AgentTurnEnded`, assert dispatched exactly once. Pairs with the existing "WorkspaceThread queue auto-dispatch render-altitude test" filed above — same fixture skeleton. Designer's correct behavior here is one of the chat-v2 surface's quiet differentiators; protecting it is textbook Harden coverage. (Filed from chat-UX research synthesis, 2026-05-13.)
