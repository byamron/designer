...
- **Visual baselines for the Phase 24 chat surface.** PR #120 added `ChatStreamRenderer`, `ToolUseBlock`, `ThinkingBlock`, `InterruptedMarker`, `LegacyChatBanner`, `LoadingState` behind `show_chat_v2` (default OFF). Existing baselines all flag-off, so the new surface ships unverified visually. Before flipping `show_chat_v2` ON in this Harden phase: regenerate visual-regression baselines with the flag forced on (test fixtures override `flagsStore`); confirm dark-mode + smallest-window (`960×640`) variants. (Filed from PR #120 staff-review pre-merge spot-check.)
- **`bootReplaying` LoadingState→EmptyState transition test.** PR #120's pre-merge review fixed an empty-state-during-fetch UX BLOCKER by adding the `bootReplaying` flag. A defaults test locks the contract (`bootReplaying: true` at store creation), but no render-level test exercises the LoadingState→EmptyState transition through `ChatStreamRenderer`. Add when `show_chat_v2` flips on so the test can mount `WorkspaceThread` through the new path. (Filed from PR #120 staff-review pre-merge spot-check.)
...
- **Deterministic test for `read_all` rowid tiebreaker.** PR #125 fixed a 1/10 CI flake in `projector_apply_is_idempotent_per_sequence` by changing `read_all`'s ORDER BY from `(timestamp ASC, sequence ASC)` to `(timestamp ASC, rowid ASC)` — `sequence` is per-stream so timestamp ties interleaved events non-causally and broke the projector's dedup. The fix is empirically stable (30/30 post-fix), but stability is absence of evidence, not a contract. A future refactor back to `sequence ASC` would only resurface the flake probabilistically. Add a deterministic test that forces two events into the same RFC3339 instant (via a `#[doc(hidden)] pub fn force_timestamp_for_test()` helper on `SqliteEventStore` that UPDATEs the timestamp column post-insert) and asserts `read_all` returns them in insertion order regardless. Lands as part of Phase 24I (AppCore integration harness) since both want test-only DB helpers. (Filed from PR #125 staff-review.)
- **Render-altitude test for `InterruptAnnouncement` aria-live region.** PR #125 added the assertive announcement on `AgentTurnEnded { stop_reason: Interrupted }` per spec §5.7. The contract is mechanical (announced turn-ids tracked in a ref so each interruption announces exactly once), but the test would require mounting `WorkspaceThread` with `show_chat_v2: true` + a primed `chatThread` state. Pairs naturally with the `bootReplaying` test above and the queue auto-dispatch render-altitude test below — same gate (chat-v2 path becomes testable) and same fixture skeleton. (Filed from PR #125 staff-review.)
