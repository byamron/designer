...
- **Phase 12a — Real-integration validation** (below) — validate Phase 0's assumptions against a real Claude Code install, build the Swift helper on Apple Intelligence hardware, wire the Tauri shell binary.
- **Phase 12 — Mobile** — unchanged (phase 2).
...
## Phase 12a — Real-integration validation *(next, before any shippable build)*

**Goal:** close the gap between the mock-first backend and the real runtimes it abstracts.

**Why now:** Phases 0–11 landed behind stable trait interfaces so the scaffold compiles + tests anywhere. The remaining risk is that the real Claude Code and real Foundation Models don't match the shapes we assumed. This phase collapses that gap.

**Deliverables:**

1. **Claude Code subprocess validation.**
   - Install the current Claude Code build and run `ClaudeCodeOrchestrator::spawn_team` against a throwaway team. Catalog the actual file shapes in `~/.claude/teams/{team}/` and `~/.claude/tasks/{team}/`.
   - Update `watcher::classify` to match observed paths/naming.
   - If the `claude` CLI arg shape for `team init` / `team task` / `team message` doesn't match the placeholder args in `claude_code.rs`, rewrite.
   - Add an integration test gated by `CLAUDE_CODE_INSTALLED=1` so CI stays green on machines without Claude auth.
2. **Swift Foundation Models helper build.**
   - Build `helpers/foundation` on an Apple-Intelligence-capable Mac (macOS 15+).
   - Confirm `LanguageModelSession.respond(to:)` is still the correct shape; if not, update.
   - Smoke test: `SwiftFoundationHelper.ping()` round-trips; `generate(JobKind::Recap, "...")` returns non-empty text.
   - Document the `designer-foundation-helper` binary path in `AppConfig` defaults.
3. **Tauri shell wiring.**
   - Add `tauri = "2"` to `apps/desktop/src-tauri/Cargo.toml`, scaffold `tauri.conf.json`, and register `#[tauri::command]`s that delegate to `designer_desktop::ipc::cmd_*`.
   - Author a restrictive Tauri allowlist (FS: `~/.designer/` + linked repo roots only; shell: `git` + `gh` + `claude` + helper path; no network).
   - Hook the `Updater` trait to Tauri's updater plugin with user-consent install prompts only.
4. **Perf pass.**
   - Measure cold start, idle memory, streaming load on a real Tauri build. Target: cold start <1.5s, idle memory <200MB.
   - Compare the full-log-replay fallback in `sync_projector_from_log` vs. the per-append `apply`; decide whether to incrementalize replay.

**Done when:** a Rust integration test spawns a real Claude Code team + observes actual events; a second test round-trips through the built Swift helper; the Tauri binary opens a window showing the React app reading from a real `AppCore`; perf budget measured.

---

## Phase 12b — Hardening second pass *(after 12a)*

- Wire Mini primitives (`Box`, `Stack`, `Cluster`, `Sidebar`, `Overlay`) into the layout components — currently all CSS-grid/flex inline (see `pattern-log.md`).
- Move approval-resolution UI out of `BuildTab`'s simulated 900 ms timeout into a real user-surface (likely inside the activity spine or a dedicated inbox).
- Set `correlation_id` / `causation_id` on events triggered by other events so traces can be reconstructed.
- Auto-grow chat textarea.
- Replace manual-entropy pairing RNG fallback with a crypto-grade source (`rand::rngs::OsRng`).
- Dark-mode visual regression harness.
...
| Multi-workspace + sync ready | 6, 7 | ✅ Preliminary build |
| First user-visible surface | 8, 9 | ✅ Preliminary build |
| Design lab + polish scaffolding | 10, 11 | ✅ Preliminary build |
| **Real-integration validated** | **12a** | **Next** |
| Hardening second pass | 12b | After 12a |
| Shippable desktop beta | signing + notarization | Blocked on Apple Developer identity |
| Mobile | 12 | Phase 2 |
