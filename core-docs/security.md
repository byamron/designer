...
**Scope note on "enterprise-grade."** Designer is local-first and per-user; it does not host user accounts, so traditional enterprise identity-federation (SSO, SAML, SCIM) does not apply and is not in scope. "Enterprise-grade" in this document refers to the security *properties* an enterprise buyer expects: encryption at rest, tamper-evident audit, fleet policy via MDM, signed and provenance-attested builds, revocable credentials, and a disclosed responsible-disclosure posture. Identity lives with the user's OS account and their Claude Code install.
