...
- components-new: none
- primitives: none
- tokens: --space-1, --color-muted, --color-foreground, --accent-9, --border-thin, --radius-pill (all pre-existing). The button is a transparent icon affordance — muted at rest, foreground on hover/focus-visible, accent-tinted focus outline so a keyboard user can find it after Tab from the composer.
