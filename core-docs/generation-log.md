...
- css-modified: `.compose-dock-activity-row` (opacity baseline + `var(--motion-quick)` transition); `.compose-dock-activity-row__pulse` background `--accent-9` → `--color-muted` (spec-mandated monochrome).
- tokens-new: none
- tokens-referenced: `--motion-quick`, `--color-muted` (added/reused). Spec said `--motion-fast` for the fade and `--motion-pulse` at 1.6s; the project's existing motion tokens are `--motion-quick: 120ms` and `--motion-pulse: 1200ms`. `--motion-fast` resolved to `--motion-quick` (closest existing tier — see pattern-log entry). The 1.2s vs 1.6s pulse-duration discrepancy is filed as a Phase 24H FOLLOW-UP rather than changing the global token (the same token drives the tab-strip badge; changing it here affects multiple surfaces).
