...
- invariants: 6/6 pass on `packages/ui/styles/team-pulse.css`, `packages/ui/styles/tokens.css` (exempt), and `packages/app/src/lab/TeamColorCatalog.tsx`. `tools/manifest/check.mjs` failed on the first push (missing manifest entry for the new lab page) and was fixed in a follow-up commit; `core-docs/component-manifest.json` now carries the TeamColorCatalog entry.
...
- files: `core-docs/adr/0009-trustworthy-shipping.md` (new), `core-docs/parking-lot.md` (new), `CLAUDE.md` (added fifth principle + fifth Quality Bar item + Build/Harden note in How to Work + parking-lot row in Core Documents table), `core-docs/spec.md` (Decision 64 added to Decisions Log), `core-docs/roadmap.md` (active sequence callout at top; Phase 24 / 24H / 24I / 25 / 25H / 26 / 26H detail sections inserted before Milestones; Parked callouts added at Phase 15.H / 17 / 18 / 19 / 20 / 21.A2 detector squad / 22 headers), `core-docs/plan.md` (rewritten 517 LOC → 42 LOC).
...
- files: `packages/app/src/components/ComposeDockActivityRow.tsx` (selector returns primitive `number | null`; exit-fade state machine; `data-exiting` attribute), `packages/app/src/styles/chat.css` (opacity transition + monochrome pulse + reduced-motion override), `packages/app/src/test/compose-dock-activity.test.tsx` (5 new chat-v2 tests + chatThreadStore/flagsStore fixtures), `core-docs/component-manifest.json` (ComposeDockActivityRow entry updated for §5.2 contract + tokens), `core-docs/pattern-log.md` (motion-fast→motion-quick mapping + monochrome-pulse rationale).
