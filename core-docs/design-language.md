...
14. **Hit-target sizing:** two steps — `--target-sm` (24px) and `--target-md` (32px). Any icon-only button uses one of these as its outer hit box; the icon inside is one of the `--icon-*` steps. `md` is the default; `sm` is reserved for genuinely dense inline affordances (chip removers, inline-row controls). Anything smaller breaks tap accessibility.
15. **Text roles in app chrome:** three sizes, tight tool register — `caption` (12px, meta/labels/kbd), `body` (**14px**, default for every control/message/list-row/title-carried-by-weight), `h3` (**18px**, the one heading role — empty-state titles and the occasional surface anchor). Everything between (`lead` 18px regular reserved for hero inputs only; `h4`, `h2`, `h1`, `display`) exists in tokens for edge surfaces but must not appear in shipped chrome without a pattern-log entry. Size ratios are 1.17× (caption→body) and 1.29× (body→h3) — tighter than perfect-fourth; the register is Linear/Figma, not Notion. At this scale size alone cannot carry hierarchy; see axiom #16.
