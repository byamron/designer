...
15. **Text roles in app chrome:** three sizes in everyday UI — `caption` (12px, meta/labels/kbd), `body` (16px, the default for controls, messages, list rows, and titles carried by weight), `h3` (24px, reserved for empty-state titles and onboarding hero). Everything between (`lead`, `h4`, `h2`, `h1`, `display`) exists in tokens for edge surfaces but should not appear in shipped app chrome without an explicit amendment. Hierarchy inside the three-role band is carried by `--weight-medium` (section titles, chat author, tab title) and `--color-muted` (meta, secondary), never by introducing new sizes.
