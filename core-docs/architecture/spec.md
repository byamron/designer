...
| 30 | Track introduced as the git-bound unit below workspace | A track = one worktree + one branch + one agent team + one PR series. Tracks are ephemeral; workspaces persist across many tracks. Matches Claude Code's agent-teams primitive at the track level (single-cwd) and leaves room above for multi-track orchestration. *Implementation (Phase 13.E, 2026-04-25):* `designer_core::Track` aggregate (`id`, `workspace_id`, `branch`, `worktree_path`, `state ∈ Active|RequestingMerge|PrOpen|Merged|Archived`, PR refs); projection in `ProjectorState.tracks` / `tracks_by_workspace`; lifecycle driven by the frozen `TrackStarted/PullRequestOpened/TrackCompleted/TrackArchived` events. |
...
| 39 | Semantic summaries are written once, at write time | When an `ArtifactCreated { kind: "code-change" }` event is emitted, `AppCore::append_artifact_with_summary_hook` calls `LocalOps::summarize_row` with a 500ms deadline and stores the result on the event. The rail and collapsed-block views always read the stored summary; never regenerate on read. Per-track debounce coalesces bursts of edits into one helper call (concurrent callers within a 2s window join the in-flight watch). Helper-down or >500ms timeout uses a deterministic 140-char ellipsis truncation; late helper returns emit `ArtifactUpdated` for the owner artifact. Wired in Phase 13.F (2026-04-25); contract documented in ADR 0003. |
| 40 | Project Home tab is three surfaces — Recent Reports / Roadmap / Designer Noticed | Today's flat list of operational state doesn't answer "where are we relative to the plan." The three-surface composition answers the manager's three opening questions in order: *what's new*, *where are we*, *what should we standardize*. Logged when Phase 22 picks up. |
| 41 | Project altitude activity spine collapses by default on the Home tab | The Roadmap surface absorbs project-altitude awareness; the spine remains primary at workspace altitude and below where no other surface duplicates "what's happening here." Toggleable for users who want chronological view. Logged when Phase 22.A lands. |
| 42 | Roadmap is a tree of nodes with stable HTML-comment anchors; depth user-controlled, leaves PR-sized | Anchors travel with the line if the user moves it. `roadmap.md` stays the source of truth (Decision 17); the canvas is a live view over it. Logged when 22.A lands. |
| 43 | **Done means shipped, not "all sub-items complete"** | A parent row with all sub-items done sits in **In Review** until its work actually ships (PR merged). Prevents the "checklist looks done but nothing is shipped" failure mode. **Two enforcement paths, both gating on shipment evidence:** (1) IPC write path — `cmd_resolve_proposal` / `cmd_set_node_status` reject with `ApplyFailed { reason: "node has no shipment recorded" }` if no `NodeShipment` exists; (2) projection path — `Track.state → NodeStatus` emits Done only when both `Track::Merged` and the matching `NodeShipmentRecorded` are present. Authored-Done in `roadmap.md` without a shipment also demotes to InReview at the projection layer, with an inline tooltip explaining why. Trust the event log; gate at every write entry point. Inline tooltip on the status circle (*"Ships when the PR for [headline] merges."*) avoids the UX surprise. Logged when 22.A lands. |
| 44 | Team identity uses inline label + pulsing colored dot — no pill on roadmap rows | The row's tint already carries the color identity; doubling it with a pill clutters the surface. Pills are reserved for the adjacent attention column where cards float against white. Each team's dot pulses at its own rate in 1.4–2.0 s, deliberately incommensurate so dots drift in and out of phase rather than syncing — communicates "alive but uncoordinated." Logged when 22.A or 22.G lands. |
| 45 | Status circles use team color for In Progress / In Review; Done is universal semantic green | Draws the eye to who's working what, while keeping "shipped" as one signal across the project regardless of team. Todo is neutral. Logged when 22.A lands. |
| 46 | Workspace and team identity uses chromatic color via amended axiom #3; chrome stays monochrome | App chrome stays monochrome. Team and team-presence introduces a constrained chromatic palette layered on top — ~16 OKLCH-derived hues with light/dark variants. Chroma deliberately below the semantic palette so success/warning/danger still pop. Logged when Phase 22.G amends `design-language.md`. |
| 47 | Adjacent attention column is the v1 surface for approvals / audit / conflicts / questions / scope-denial | Migrates "Needs your attention" off the Home tab. Gutter-pinned side comments on the canvas are deferred. Logged when 22.E lands. |
| 48 | Designer Noticed v1 reuses the existing 21.A1.2 surface as-is; five-category re-skin is deferred | Verb-baked proposal titles (*"Add Rule: …"*, *"Add Skill: …"*) already carry the action; the existing `FindingRow`/`ProposalRow` components ship the unified Dismiss / Accept primary already. The five-category display taxonomy (Skill / Rule / Anchor / Routine / Agent) was the original spec's framing but cut from v1: one of the five (Anchor) has no backing detector, and dogfood signal hasn't asked for grouping yet. Revisit only if proposal volume crosses the legibility threshold. Logged 2026-04-30. |
| 49 | Roadmap edits flow through proposal/accept under the existing autonomy gradient | Reversible status updates auto-apply under Act/Auto; structural changes (adding sub-items, renaming, restructuring, removing) always require review. Direct user edits open the markdown in a tab — never silent reformatting from the canvas. Logged when 22.D lands. |
| 50 | Linear / Notion / Jira / Asana / GitHub Projects integration is cut from v1 (interop ≠ moat) | Linear users live in Linear; Designer's value-prop is markdown-first plans living in the repo (Decision 17). A Linear-source-of-truth canvas creates two-source-of-truth confusion that can't be fully resolved at the architecture level. Cut entirely from the Phase 22 roadmap; revisit only on explicit user signal. Replaces the prior decision to ship Linear bidirectional. Logged 2026-04-30. |
| 51 | Recent Reports read state is a projection, not events; v1 is project-scoped (extends to user-scoped on team-tier) | v1 ships `read_seq_by_project: HashMap<ProjectId, EventSeq>` — single integer per project, last-seen sequence in the report stream. Implicit advance on inline expand or tab open; explicit *Mark all read* footer advances to head. **Why project-scoped at v1:** there is no `UserId` concept until team-tier (Phase 17). Shaping the projection key as `(UserId, ProjectId)` today bakes in infrastructure that doesn't exist. When team-tier lands, extend the key — additive schema change, no event-log rewrite (projection rebuilds from the existing report stream). **Why a projection, not events:** per-report `ReportRead` events would bloat the event log linearly with reports × users; a single seq-per-project projection captures the same information in O(1) state. Disclosure is two-step (inline summary → click to expand inline); the hover-preview middle step from the original spec is dropped as undiscoverable friction. Logged when 22.B lands. |
| 52 | Reports ship one voice (high-level / manager) at v1; multi-voice deferred behind a settings affordance | Three voices (high / medium / technical) at write time was rejected as feature creep: the primary user is one person (the manager), the Foundation Models cost is real, and dogfood hasn't asked for engineer-facing copy on the Home tab. If signal lands, add a settings toggle "show technical detail" — generates `summary_technical` at write time and a per-report toggle. Two voices, not three. Single high-level call extends the existing `summarize_row` hook (Decision 39). Logged when 22.B lands. |
| 53 | Live claims and `NodeShipment` historical attribution are separate projections | Live presence cleans up on track Merge so the canvas stays current; shipping history (`NodeShipment`) persists permanently for audit and "Shipped here" badges. `NodeShipmentRecorded` is emitted in the same transaction as `Track::Merged` so the IPC enforcement of the Done = shipped invariant (Decision 43) has the shipment to look up. Multi-claim ordering is deterministic: stacked team labels sort by `claimed_at` ascending, ties break by `team_id` lexicographic. Logged when 22.I lands. |
| 54 | Sub-agent claims default to inheriting the parent track's claim; explicit anchor refs in agent-team task lists update the assigned teammate's claim | Keeps the claim model honest at sub-agent altitude without requiring agents to do extra bookkeeping by default. Logged when 22.H lands. |
| 55 | `Track.state` projects to `NodeStatus` with a documented mapping | Active→InProgress, RequestingMerge→InReview, PrOpen→InReview, Merged→Done (gated on Decision 43), Archived→Canceled (when archived without merge) or untouched (when archived after merge). Three different status vocabularies (node statuses, track states, finding severities) on one canvas was unsustainable; the projection collapses node + track into one user-visible vocabulary. Documented in `core-docs/roadmap-format.md`. Logged when 22.A lands. |
| 56 | Reports overflow stays inline; no project-level Reports tab kind | *Show all (N)* expands the section to its full list inline. A "Reports tab" would silently violate Decision 36's retired-templates discipline (no project-altitude tab kind exists today). Decision-log entry replaces the original spec's "Past ~8, route to a project-level Reports tab." Logged when 22.B lands. |
| 57 | Unified Dismiss / Accept primary across all Home-tab accept-patterns | Three different accept-patterns on one tab (canvas edit proposals, attention-card binary actions, Designer Noticed cards) was incoherent. v1 collapses to one binary (Dismiss / Accept) with verb-customized labels per context (Deny / Approve, Hold / Merge, etc.). Edit elevates to a separate inline action only when a diff is involved (22.D). Snooze drops to a kebab-menu item. Logged when 22.E lands. |
| 58 | Confirmation animations stay snappy, never spring (axiom #5 preserved) | Original spec called for a celebratory spring scale-pop on merge-confirm. Axiom #5 in `design-language.md` explicitly prohibits crossing into expressive/spring territory; introducing spring would require amending #5 alongside #3, which is a much larger move than introducing chromatic team identity. Snappy fill (`--motion-emphasized` ease-out, no scale) carries the "your action took" semantic without violating the motion personality. Logged when 22.E lands. |
| 59 | Phase 22 chromatic palette uses `--team-1`..`--team-16` tokens, namespaced separately from `--accent-*` | Accents stay monochrome per axiom #3 even after the chromatic-identity amendment. Token names are explicitly **not** `--accent-team-*` — they are not substitutes for accents and never compose into chrome. Sub-row tints use a paired `--team-tint-light` (~7% alpha) and `--team-tint-dark` (~10% alpha) so dark mode keeps a perceptible tint without breaking contrast. Logged when 22.G amends `design-language.md`. |
| 60 | All Phase 22 surfaces honor `prefers-reduced-motion`; verified by `audit-a11y` before merge | Pulses → static dot; conic-arc state transitions → instant flip; expand/collapse → instant; merge-confirm fill → instant green; card slide-out → instant removal. No exceptions. Per axiom #5, reduced-motion fallback is required, not optional. Logged when 22.A or 22.E lands. |
| 61 | Multi-claim node status is the maximum claiming-track state under an all-must-ship Done gate | When N tracks claim the same node, the projected `NodeStatus` is the max of `{Backlog < Todo < InProgress < InReview < Done < Canceled}` across claims, **except** Done is only emitted when *every* claim has a corresponding `NodeShipment`. Any unshipped claim demotes the projection to InReview. Rationale: multi-claim is parallel work toward one node, not a race; the node is Done only when all of it has shipped. Documented in `roadmap-format.md`. Logged when 22.A lands. |
| 62 | Primary node claims derive from `TrackStarted.anchor_node_id`; sub-agent claims derive from task-list anchor references in 12.A's watcher | `TrackStarted` gains an additive `anchor_node_id: Option<NodeId>` field per the Lane 0 ADR; `cmd_start_track` populates it when called with an anchor argument. The `node_to_claimants` projection derives `NodeClaim` records from this field. Mid-flight re-claiming a track to a different node is **out of scope for v1** — end the track, start a new one. Sub-agent claim discovery (Phase 22.H) is separate: the 12.A task-list watcher must expose task body text so the projection can extract `<!-- anchor: ... -->` references; if today's watcher event variant lacks body text, an additive field lands per the Lane 0 ADR. Logged when 22.A and 22.H land. |
| 64 | Trustworthy shipping: Build/Harden alternation + parking-lot mechanism (D2026-05-04) | Active roadmap accreted 25 phases over two months; each ship left foundation gaps that the next dogfood pivot exposed. ADR 0009 introduces a fifth product principle ("Shipped state is trustworthy") and two structural changes that enforce it: (1) the active roadmap alternates Build phases (one feature, one track) with Harden phases (no new features — only test coverage, friction closure, design-language enforcement, and demo gatekeeping), and (2) `core-docs/parking-lot.md` holds deferred phases with friction-driven primary triggers and time-based fallbacks so ambition is preserved without the cognitive load of a 25-phase active surface. Verification per release tag is a checked-in golden-path screencast bound to a Playwright test that gates the release; macOS visual-regression CI is itself parked until cost is justified. **How to apply:** before adding a phase to active roadmap, ask "does this Build cycle have an open feature already?" If yes, the new work is the *next* Build, not a parallel track. When a phase isn't load-bearing for the current cycle, file it in `parking-lot.md` with a friction-log primary trigger; never let it linger in `roadmap.md` as "active later." Hidden-but-emitting events (Designer-Noticed detectors, etc.) are an explicit Designer convention per ADR 0009 §3, not dead code. Logged 2026-05-04. |
| 63 | Settings is global-only; per-project concerns live in Project Home (D2026-05) | Settings used to be a single full-page surface mixing global concerns (Appearance, Keychain, Activity, cost-chip preference) with per-project concerns (the linked-repo display, which is per-workspace and project-scoped). Two friction reports landed simultaneously asking to (a) split scopes and (b) add a project unlink affordance — fixing them together. **Rule:** Global settings (theme, density, Claude Code account, keychain, cost-chip toggle, feature flags, friction triage, "Designer noticed") stay in `SettingsPage`. Per-project settings (linked repo, autonomy override, project-scoped report preferences) live in **Project Home** (`HomeTabA`). The "Repository" row in Settings → Account moved to Project Home as a `ProjectRepoSection` block; the new `cmd_unlink_repo` IPC + `WorkspaceWorktreeDetached` event are the inverse of `cmd_link_repo`. Unlink severs Designer's pointer; the repo on disk is untouched. **How to apply:** when adding a new setting, ask "is this scoped to one project or to the whole installation?" Per-project → Project Home. Global → Settings. If a setting is conceptually per-project but the data model only supports per-workspace (as today's repo link does), the project-home surface fans out across the project's workspaces on action. Logged 2026-05-01. |
| 65 | Approvals are project artifacts surfaced via a global inbox (D2026-05-06) | An approval is a first-class project artifact — id, timestamp, source chat, status — with a lifecycle of *open → resolved*. It surfaces in a **global inbox** visible from the cockpit (sidebar count, home-page list, dedicated inbox view) wherever the user is in the product. The user can act on the approval from any inbox surface *or* jump to source in the originating chat for full context. Inbox is where approvals are *acted on*; chat is where they're *born and contextualized*. **Sharpens Decision #47** (which named the *adjacent attention column* as the v1 surface) by giving approvals a generalized artifact shape and lifecycle that holds across multiple inbox surfaces, not just one. With many parallel agent teams, per-chat-only approval surfaces fail at scale: the user can't tab through every chat to find pending decisions; visibility, recognition over recall, the Zeigarnik effect, and Tesler's conservation of complexity all align. Established cross-domain pattern (GitHub PR queue, email inbox, Linear, OS notification center) — not invented. **Implementation-level questions deferred** (chat-side rendering vs inbox-side rendering, badge/notification model, inbox layout) — designed in a future iteration cycle. Originated in the Mini × Taste taste-loop, cycle 5. |
| 66 | Reports are frozen snapshots with append-only supersession (D2026-05-06) | A report is a **frozen snapshot** at the moment it's created — its content never changes in place. When work changes the state a report describes, a **superseding report is automatically created** that links back to its predecessor; the newer report visually takes the foreground; the predecessor remains accessible in a clearly-timestamped history chain. *Visual implication:* at any version, the user sees the current version prominent + a small "← previous version" affordance + clear timestamps + each predecessor labeled as outdated. **Extends Decisions #51/#52/#56** (read-state projection, one-voice-at-v1, no project-level Reports tab) — none of those speak to the freeze-and-supersede shape. Avoids the failure mode the user named explicitly: stale documentation that misleads. Trust through legibility (Principle 5), recognition over recall, and Tesler's conservation of complexity all align — editorial decisions about "update in place vs new doc?" disappear; the system always supersedes; the user always sees current; history is preserved as context. Established cross-domain pattern (Wikipedia article history, git commits, Linear edited-at trails, Notion page history). **Implementation-level questions deferred** (what triggers a supersession — agent decides? code-change downstream? chain visualization; whether reports can branch into multiple successors) — designed in a future iteration cycle. Originated in the Mini × Taste taste-loop, cycle 5. |
