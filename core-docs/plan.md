...
|---|---|---|
| **24 — Chat pass-through** | In flight, **mid-cycle**. All work behind `show_chat_v2` (default OFF) until the renderer + tests catch up to flip-on quality. **Shipped:** steps 1–3 (PR #119), step 4 (PR #130 — user-only dispatch contract pinned), steps 5–6 (PR #120 — chat-thread reducer + renderer), step 7 (PR #124 — queue + stop-and-send + SendMenu), steps 8–9 + §5.7 (PR #125 — Esc priority chain, SIGINT, assertive announcement), step 10 (PR #131 — render-time activity indicator), step 11 (PR #132 — detector dual-shape recognition), step 12 (this PR — §5.6 error-state copy: `ErrorMarker` + `MaxTokensMarker` inline siblings of `InterruptedMarker`, assertive `ErrorAnnouncement` aria-live region carrying §5.6 row-2 copy, `humanize_dispatch_error` second-fail copy aligned to spec; new `--color-foreground-muted` + `--color-warning` semantic tokens). **Remaining:** step 13 (A1–A12 acceptance tests). Then the coalescer can be deleted (step 3 cleanup) and `show_chat_v2` can default ON. | 1:1 stream-json projection per ADR 0008. Coalescer deletion. Renderer rewrite, queue UX, ESC + SIGINT, detector updates, error copy, A1–A12 tests, Mini docs. |
...
- **Phase 24 sequencing:** translator + bridge + renderer + queue + Esc/SIGINT + user-only dispatch contract + render-time activity indicator + detector dual-shape recognition + §5.6 error-state copy all shipped behind `show_chat_v2` default OFF (PRs #119, #120, #124, #125, #130, #131, #132, and step-12 PR). Remaining workspace step: 13 (A1–A12 acceptance tests). Tests are the load-bearing gate before `show_chat_v2` defaults ON — see the Phase 24H roadmap entries for the render-altitude tests (`bootReplaying`, queue auto-dispatch, `InterruptAnnouncement`, and now the §5.6 markers + `ErrorAnnouncement` per Step 12) that gate on the flag becoming testable. The coalescer + per-tab `first_seen_at` tables in `core_agents.rs` (step 3) remain in place until `show_chat_v2` defaults ON.
