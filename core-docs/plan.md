...
**Phase 13.E shipped (2026-04-25, PR #16).** Track aggregate + projection landed (Active → PrOpen → Merged → Archived; `RequestingMerge` reserved as in-process-only). `cmd_link_repo` canonicalizes via `fs::canonicalize`, validates via `git rev-parse --is-inside-work-tree`. `cmd_start_track` creates per-track worktrees under `<repo>/.designer/worktrees/<track-id>-<slug>`, seeds `core-docs/{plan,spec,feedback,history}.md`, commits them on the new branch, with full rollback if any step after `init_worktree` fails. `cmd_request_merge` runs `gh pr create` under a 30s timeout and dedupes concurrent calls via an in-flight set. Edit-batch coalescer emits one `code-change` artifact per unique per-file `+a:-r` signature. Branch names are validated for argument-injection (`-`, whitespace, control chars, ref-meta). Per-repo async mutex linearizes concurrent `start_track`. RepoLinkModal has a Tab/Shift-Tab focus trap and dismisses on `onClick` (not `onMouseDown`). Quality gates green: 32 desktop tests, 7 designer-core, 3 designer-git URL-extractor, 18 vitest. See `history.md` for the full entry + review-pass detail.

**13.D / 13.F / 13.G / 13.H remain parallelizable.** Each track emits into the block registry — no UI work required in those tracks:
...
- **13.G — Safety surfaces + Keychain** ✅ *landed 2026-04-25*: production `InboxPermissionHandler` swapped in for `AutoAcceptSafeTools`; cost chip + Keychain status row + scope-denied path all wired. See `history.md` 13.G entry.
