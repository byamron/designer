...
**Phase 23 ship status (2026-05-03).** Wave 1 + Wave 2 both landed: 23.A (PR #91), 23.C trail (PRs #92 → #94 → #97 — see below), 23.D + follow-up (PRs #90, #93), 23.E per-tab subprocess (PR #95). Wave 3 (**23.B** activity indicator) is now unblocked — its activity event can carry `tab_id` since 23.E shipped. The 23.C trail is the longest in this phase: PR #92 shipped expand-to-payload itself, three review rounds caught state-update-after-unmount + missing loading state + missing aria-live + invalid `min-height` calc + engineer-y error copy + missing chevron discoverability + transient-error stickiness, all fixed across PRs #94 (region wrapper + error copy + manifest refresh) and #97 (chevron + Try-again). Two parked items remain in `roadmap.md` §"Phase 23.C follow-ups": 23.C.f2 (`BlockProps.expanded` adoption — speculative, awaiting a parent-collapse consumer) and 23.C.f3 (coalesce same-tool rows — explicit Phase 23 v2 polish per "Out of scope (v1)"). f1 (chevron) and f4 (retry-on-error) shipped in PR #97.
