...
- **Phase 24 sequencing:** translator + bridge + renderer + queue + Esc/SIGINT + user-only dispatch contract all shipped behind `show_chat_v2` default OFF (PRs #119, #120, #124, #125, and step-4 PR). Remaining workspace steps: 10 (render-time activity indicator), 11 (detector dual-shape recognition), 12 (§5.6 error-copy mapping), 13 (A1–A12 acceptance tests). Tests are the load-bearing gate before `show_chat_v2` defaults ON — see the Phase 24H roadmap entries for the three render-altitude tests (`bootReplaying`, queue auto-dispatch, `InterruptAnnouncement`) that gate on the flag becoming testable. The coalescer + per-tab `first_seen_at` tables in `core_agents.rs` (step 3) remain in place until `show_chat_v2` defaults ON.
