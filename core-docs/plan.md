...
**Phase 21.A1 — Learning layer foundation shipped 2026-04-27 (PR #33; rebased after 13.K).** Foundation crate `crates/designer-learn` (locked `Detector` trait, `Finding` / `Severity` / `ThumbSignal` types in `designer-core`, `SessionAnalysisInput` builder, Forge-migrated thresholds + keyword corpora with citations, `NoopDetector` worked example, fixture-test harness, load-bearing `CONTRIBUTING.md`). Additive `EventPayload::FindingRecorded` + `FindingSignaled` per the Lane 0 ADR addendum. `core_learn.rs` wires `report_finding` / `list_findings` / `signal_finding` onto AppCore + a `forge_present` probe that defaults overlapping detectors to disabled when `~/.claude/plugins/forge/` is present. New `commands_learn::cmd_list_findings` / `cmd_signal_finding` IPC + `FindingDto` / `SignalFindingRequest` DTOs. Settings → Activity now hosts both Friction (#34) and "Designer noticed" (this PR) sub-pages via a `SegmentedToggle`. Re-uses 13.K's frozen `Anchor` enum verbatim — no shape redesign. Phase 21.A2 (the ten parallel detectors) is now unblocked.

**Track 13.K — Friction shipped 2026-04-27 (PR #34).** In-app feedback capture surface so dogfood friction goes from "this affordance feels wrong" to a markdown record + GitHub issue in <5s. Bottom-right `FrictionButton` (⌘⇧F), `SelectionOverlay` with smart-snap to component roots (Alt overrides), three working screenshot inputs (paste / drop / file picker), Settings → Activity → Friction triage page with retry + mark-resolved. `Anchor` enum frozen for reuse by 15.H + 21.A1; `EventPayload` gained four `Friction*` variants per the ADR 0002 addendum. Backend pipeline: synchronous markdown record + content-addressed PNG → `FrictionReported` → background `gh gist create --secret` (PNG-header probe; downscale to 1920px if wider) → `gh issue create --label friction` → `FrictionLinked` (or `FrictionFileFailed`). `.surface-dev-panel` relocated to bottom-left so the capture corner is unambiguously the user's. **Unblocks Lane 3 (dogfood-driven Phase 15.J).** Auto-capture (Tauri `webview.capture()`) and a stream-subscribed "Filed as #N" toast are explicit v2 follow-ups.
...
**Dogfood Push shipped + friction loop iteration (2026-05-01 → 2026-05-02).** The DP-A/B/C bundle landed in PR #63: `tauri-plugin-updater` on `v*` tags + `UpdatePrompt`, pass-through chat with one-line artifact references that focus the spine, and the *Feature readiness* audit table (19 prod / 1 flag / 12 hide). Release-pipeline gaps that surfaced on the v0.1.0 tag-push were fixed across PRs #65 (notarization wiring + Release publish permissions), #66 (`createUpdaterArtifacts: true`), #69 (routine-release docs + sand-tile icon), #71 (d-mark icon), and #74 (Phase A test floor — updater conf, safety smoke, IPC envelope, update-prompt). The friction loop matured across PRs #67 (CLI `designer friction list/address/resolve/reopen` + fs-watcher), #70 (batch-copy + first wave of fixes + dispatch packs), and #76 (Enter-to-send, pulse gating, file-drop + submit smoothness). PR #72 added a daily-rotating file appender + auto-recover stale claude handle to make bundled-only failure modes observable. PR #75 closed the high-attention tab-UI friction (close-tab IPC + ⌘W + no-flash on switch + per-tab drafts). PR #78 (settings-scope-unlink, currently open) splits Settings into global-only and moves per-project concerns to Project Home, ships the `cmd_unlink_repo` affordance addressing two friction reports. Phase 22 — Project Home redesign — is now the next-up architectural body of work; spec + roadmap + plan landed in PR #62 with eight independently shippable sub-phases; first slice recommendation is 22.G + 22.B + 22.A + 22.I behind a feature flag. See `history.md` 2026-05-01 → 2026-05-02 entries for each PR's why/decisions/tradeoffs.
...
**Phase 23 — Chat UX hardening promoted to active 2026-05-02.** PR #87 stripped the experimental agent-teams framing from default chat (closes friction `019dea66`, `019dea67`, `019dea69`, `019dea6a-0f1d`) but dogfood the same day surfaced the next layer: tool-use rows landed *after* the agent text + user reply (timestamp drift from the coalescer's idle-flush), tab switching tore down the artifact-stream listener so the agent felt paused, and a single Claude subprocess per workspace meant tabs shared a session — confusing both the model and the user. Phase 23 closes all five gaps (23.A coalescer first-token timestamp, 23.B activity indicator, 23.C tool-use expand, 23.D tab-switch keeps thread mounted, 23.E per-tab Claude subprocess). Wave 1 (23.A + 23.C + 23.D) is file-disjoint and dispatches in parallel; 23.E lands solo (largest backend change; touches `core_agents.rs` + `claude_code.rs`); 23.B follows 23.E because its activity event carries `tab_id`. Detail in `roadmap.md` § Phase 23.

**Phase 24 in flight 2026-05-04 (`phase-24-chat-pass-through`).** Architectural chat pass-through pass per `core-docs/phase-24-pass-through-chat.md` + ADR 0008. Step 1 of 14 landed: event vocabulary additively extended in `crates/designer-core` — six new `EventPayload` variants (`AgentTurnStarted`, `AgentContentBlock{Started,Delta,Ended}`, `AgentToolResult`, `AgentTurnEnded`) plus the supporting types (`ClaudeMessageId`, `ClaudeSessionId`, `AgentContentBlockKind`, `AgentStopReason`, `TokenUsage`) in `domain.rs`. `CostRecorded` gained additive `tab_id` / `turn_id`. Envelope version 2 → 3. `MessagePosted` doc-comment flags `Actor::Agent` author as deprecated for forward writes (`Actor::User` stays canonical). Field name `block_kind` (not `kind`) on `AgentContentBlockStarted` dodges the outer `tag = "kind"` serde collision — same precedent as `artifact_kind`. Round-trip tests + a legacy-cost-record decode test pin the additive contract. `cargo check --workspace` clean; full test suite green. Steps 2-14 (translator rewrite, coalescer deletion, renderer rewrite, queue UX, ESC + SIGINT, detector updates, error copy, `show_chat_v2` flag, A1-A12 tests, Mini docs) follow in this branch behind the same flag default OFF.

**Phase 23 Wave 1 + Wave 2 shipped 2026-05-03.** All three Wave 1 sub-phases plus follow-ups landed in a single morning: PR #91 (23.A first-token-timestamp coalescer), PR #90 (23.D tab-switch keep-mounted) + PR #93 (23.D follow-up — `hasStarted` resync, ComposeDock keyed-by-stateKey, `initialPaint` re-arm, the two BLOCKERs PR #90's brief-bound scope deferred). Per-tab model + effort + attachments persistence captured as Wave 2 / 3 follow-ups. Wave 2 (**23.E** per-tab Claude subprocess) shipped in PR #95 with PR #98 follow-ups (migration banner + `ChannelClosed` copy). Wave 3 (**23.B** activity indicator) is now unblocked — its activity event can carry `tab_id` since 23.E shipped. Documentation gap noted 2026-05-03: PRs #84–#94 had not been logged in `history.md` or reflected in `plan.md` Current Focus until the docs-backfill PR (#96).

The 23.C trail (PRs #92 → #94 → #97) is the longest in this phase: PR #92 shipped expand-to-payload itself, three review rounds caught state-update-after-unmount + missing loading state + missing aria-live + invalid `min-height` calc + engineer-y error copy + missing chevron discoverability + transient-error stickiness, all fixed across PR #94 (layout-stable region wrapper, screen-reader region semantics, "Nothing to show." error state) and PR #97 (chevron discoverability + Try-again retry on error). Two parked items remain in `roadmap.md` §"Phase 23.C follow-ups": **23.C.f2** (`BlockProps.expanded` adoption — speculative, awaiting a parent-collapse consumer) and **23.C.f3** (coalesce same-tool rows — explicit Phase 23 v2 polish per "Out of scope (v1)"). 23.C.f1 (chevron) and 23.C.f4 (retry-on-error) shipped in PR #97.
...
- ~~**[P0] Track 13.K — Friction.**~~ **Shipped 2026-04-27 (PR #34).** Anchor enum + EventPayload variants + IPC + Settings IA + frontend surfaces all landed. Auto-capture + stream-subscribed "Filed as #N" toast deferred to a v2 follow-up. Lane 3 unblocked.
- **Phase 21.A1 — Learning layer foundation.** ✅ Shipped 2026-04-27 (PR #33; rebased on top of 13.K). Locked the Detector trait, Finding/Severity/ThumbSignal types in `designer-core`, `SessionAnalysisInput` builder, Forge-migrated thresholds, fixture harness, and CONTRIBUTING.md. Re-uses 13.K's frozen `Anchor` enum verbatim. Phase 21.A2 unblocked.
- ~~**Lane 1 prereq: `data-component` annotation.**~~ *Shipped 2026-04-26 (PR #29, branch `friction-anchors`).* `data-component="<ComponentName>"` on the topmost rendered DOM element of every top-level component in `packages/app/src/{layout,components,blocks,lab}/` (~25 sites including `AppShell`, `ProjectStrip`, `WorkspaceSidebar` + `WorkspaceRow`, `MainView`, `ActivitySpine`, `TabLayout`, `QuickSwitcher`, `SettingsPage`, `ComposeDock`, `RepoLinkModal`, `CreateProjectModal`, `AppDialog`, `Onboarding`, `IconButton`, `Tooltip`, `PrototypePreview`, plus all 10 block renderers). Convention documented in `pattern-log.md` (2026-04-26 entry). Track 13.K's Friction resolver can now anchor against component identity rather than brittle structural CSS paths.
...
- ~~**Phase 21.A1.1 — Findings on workspace home + budget + dedup + calibrated badge**~~ **Shipped 2026-04-27 (PR #37).** "Designer noticed" lives at the bottom of the project home tab (top-8 severity-sorted live feed); Settings → Activity → Designer noticed is the full archive. Calibrated 👍/👎 badge on thumbed findings (`list_signals` projection over the System stream, last-write-wins). Sidebar Home button shows an unread badge counting `finding_recorded` events since `noticedLastViewedSeq`. `DetectorConfig.max_findings_per_session: u32` (default 5) enforced atomically in `report_finding` (race-free reserve-and-refund pattern, verified by `report_finding_cap_holds_under_concurrency`). Write-time dedup against the open findings projection no-ops on duplicate `window_digest` and refunds the cap. Severity-calibration §6 added to `CONTRIBUTING.md`.

**Wave 2 — sequential** *(depends on Wave 1)*:

- ~~**Track 13.M — Friction trivial-by-default UX**~~ **Shipped 2026-04-28.** Default flow is ⌘⇧F → composer mounted bottom-right with body autofocused; ⌘↵ submit; ⌘⇧S captures the viewport via `cmd_capture_viewport` (Tauri 2 has no built-in webview capture, so we shell to macOS `screencapture` with the window's screen-rect; non-macOS hosts return a clear error). Composer hides for one paint frame before capture so it doesn't appear in its own screenshot. Selection mode is opt-in via 📍 button or ⌘.; the 600ms outside-click grace is replaced by a 50ms suppression window after arming. Persistent key-hint footer in the composer (⌘↵ submit · ⌘⇧S screenshot · ⌘. anchor · ESC dismiss) and persistent legend on the selection banner. FrictionButton visually demoted (smaller, lower-contrast). Stream-subscribed toast updates from "Filed locally" → "Filed as #N" once `friction_reported` lands in the projection.
...
- **Phase 21.A1.2 — Surface rework: proposals over findings, boundary-driven cadence** *(architecture correction surfaced 2026-04-28; ~2 days, one agent)*. Decouples detector firing from UI updates: detectors keep firing continuously (cheap, accurate), but the user-facing "Designer noticed" surface only refreshes at `TrackCompleted` and first workspace-home view per day. The unit the user thumbs becomes a **proposal** (a recommendation), not a finding (an observation). Findings move into a collapsed evidence drawer under each proposal. Ships the `Proposal` type + `ProposalEmitted/Resolved` events + a stub synthesizer (groups findings by detector/workspace) so the surface contract is forward-compatible with Phase B's real LLM synthesis. Touches `core_learn.rs`, new `core_proposals.rs`, `commands_learn.rs`, frontend `DesignerNoticedHome.tsx` + `DesignerNoticedPage.tsx`. No conflict with detector-squad work below — different layers. Detail in `roadmap.md` § Phase 21.A1.2.
- **Phase 21.A2 — Detector squad.** Spin up one agent per detector. CONTRIBUTING.md (from 21.A1, with the 21.A1.2 "Summary copy" addendum) locks the shapes. Detector summaries are **evidence text** (clinical, passive voice, describe the pattern), not user-facing lines — Phase B will synthesize the user-facing recommendation copy from these. Recommended order — Designer-unique first since they exploit Designer's event-store advantage over Forge's plugin position:
  - `repeated_correction` (fastest signal — corrections are loud)
  - ~~`approval_always_granted` *(uses `ApprovalRequested` events)*~~ **Shipped 2026-04-29 (PR #44).** Approval class = `(workspace_id, tool, canonical_input)`; `Notice` finding when ≥5 grants and 0 denials. `window_digest` is keyed on class identity (not the evidence list) so the chokepoint dedup behaves correctly across runs. Designer-unique → not in `FORGE_OVERLAP_DETECTORS`.
  - ~~`scope_false_positive`~~ ✅ **Shipped 2026-04-29 (PR #46).** Pairs `ScopeDenied` events with subsequent `ApprovalGranted` events whose request summary names the same canonical path; emits a `Notice` finding with confidence clamped to `[0.5, 0.85]` for use under a `scope-rule-relaxation` proposal. Lexical path canonicalization (no FS touch) plus `*` / `**` glob-prefix stripping for summary matching. Designer-unique — always runs.
  - ~~`cost_hot_streak`~~ ✅ **Shipped 2026-04-29 (PR #47).** Streaming pass over `CostRecorded` events with a bounded rolling window; emits an `Info` finding when a recurring task class trips ≥1.5× rolling p90 with ≥3 prior class occurrences and ≥10 baseline entries. Designer-unique — Forge never sees `CostRecorded`.
  - ~~`compaction_pressure`~~ ✅ **Shipped 2026-04-29 (PR #54).** Per-workspace idle-gap session segmentation on `MessagePosted`; emits a `Notice` finding when `/compact` appears across ≥3 sessions in the trailing 7 days (anchored on the latest input event for replay-determinism). Evidence: `MessageSpan` per qualifying compact, capped at 5 anchors; `window_digest` keys on the full uncapped list so dedupe stays stable. First detector to loop per-workspace and carry `Some(workspace_id)` from the loop key, so it works correctly on project-wide bundles. Designer-unique — Forge never sees user-typed slash commands.
  - ~~`multi_step_tool_sequence`~~ ✅ **Shipped 2026-04-29 (PR #55).** Walks `ArtifactCreated` events authored by the agent, parses the verb-first card title back to a canonical tool name (`Read`/`Write`/`Edit`/`Search`/`Bash`/`Used <X>`), and emits a `Notice` finding when a length-3 sliding window over a session's tool-use run repeats across ≥`min_sessions` distinct user-led sessions. User `MessagePosted` events delimit sessions and break runs; pre-message tool runs are discarded (no anchor target). Evidence anchors capped at 5 per kind; the summary keeps the uncapped session + occurrence counts. Forge-overlap → already in `FORGE_OVERLAP_DETECTORS`; AppCore disables it when Forge is co-installed. `VERSION` bumps to 2 when the typed tool-call event variant lands.
  - ~~`repeated_prompt_opening`~~ ✅ **Shipped 2026-04-29 (PR #53).** Clusters per-workspace first-user-message openers by Jaccard similarity (≥0.5) over lowercased, punctuation-stripped token sets; emits a `Notice` finding when a cluster reaches `min_occurrences` (default 4 per `SKILL_DEFAULTS`). Listed in `FORGE_OVERLAP_DETECTORS` — AppCore wiring auto-disables when Forge is co-installed. Threshold cited from Forge `analyze-transcripts.py` L1231 (Forge ships 0.30; Designer tightened to 0.50 for the cockpit's stricter attention budget). Wraps the analysis pass in a 250 ms `tokio::time::timeout` per CONTRIBUTING §"partial-failure containment".
  - ~~`domain_specific_in_claude_md`~~ ✅ **Shipped 2026-04-30 (PR #57).** Reads `<project_root>/CLAUDE.md` and emits one `Notice` finding per line that substring-matches a keyword in `DOMAIN_SPECIFIC_CLAUDE_MD_KEYWORDS` (file extensions `.tsx/.rs/.py/.go`, framework names `tailwind/radix/tokio/pytest/vite`, and directory anchors `packages/app//apps/desktop//src-tauri//crates/`). Confidence fixed at 0.6 — heuristic. Output kind is `rule-extraction`; Phase B's synthesizer turns each finding into a scoped `.claude/rules/<name>.md` proposal with `paths:` frontmatter narrowing the rule to the file family it concerns. Per the task spec, the detector emits one finding per matching line and lets `core_learn::report_finding` enforce `max_findings_per_session` at the chokepoint. Listed in `FORGE_OVERLAP_DETECTORS` — AppCore wiring auto-disables when Forge is co-installed.
...
- **Phase 22.G — Color system (axiom #3 amendment)** *(pulled forward 2026-05-02; ~1–2 days, one agent)*. Tokens-only work: 16-hue OKLCH palette stepped at fixed chroma + lightness with light + dark variants, two derivable values per hue (soft tint ~7–8% alpha + inked color), pulse-rate generator (1.4–2.0s incommensurate; CSS keyframes only, never JS), workspace color picker in project settings, `prefers-reduced-motion` static-dot fallback. Touches `packages/ui/styles/tokens.css`, `core-docs/design-language.md` (axiom #3 amendment), `core-docs/pattern-log.md` (semantic vs identity color separation). New tokens are `--team-1`..`--team-16`, explicitly **not** `--accent-team-*` — accents stay monochrome. File-disjoint from all other Lane 2 work. Acceptance tests T-G-1..T-G-5 in `roadmap.md` § Phase 22.G gate the merge.
- **Phase 22.B — Recent Reports redesign** *(pulled forward 2026-05-02; ~2–3 days, one agent)*. Extends the `report` artifact with `summary_high` (additive, optional) generated at write time via the existing `summarize_row` hook (no new IPC). Adds `read_seq_by_project` projection (single integer per project; **no** `ReportRead` events — avoids O(reports × users) event-log bloat). Source classification (Feature / Fix / Improvement / Reverted) by the local model. Two-step disclosure: inline 1–2 line summary → click expands inline; *Open in tab* is the only path that creates a tab. Three reports default; *Show more* expands in place; **no project-level Reports tab** (Decision 56). Touches `crates/designer-core/src/artifact.rs` and adjacent, `apps/desktop/src-tauri/src/core_local.rs` (summarize_row extension), `packages/app/src/home/HomeTabA.tsx` (Recent Reports section replacing current report rendering — builds on DP-B's chat-surface restructure already shipped in PR #63). Acceptance tests T-B-1..T-B-5 in `roadmap.md` § Phase 22.B gate the merge.
...
- **Phase 22 — Project Home redesign.** Three-surface Home tab (Recent Reports / Roadmap / Designer Noticed) per `roadmap.md` § Phase 22. v3 spec reviewed 2026-05-02; first-principles cuts hold. **22.G + 22.B promoted to Lane 2 (2026-05-02)** — see Lane 2 entries; remaining sub-phases stay deferred. Recommended sequence for the rest, picked up after dogfood signal: **22.A** (Roadmap canvas foundation — parser + node/claim/shipment data model + renderer + `Track.state → NodeStatus` projection + Done-shipped IPC enforcement) → **22.I** (shipping history + "Shipped here" badges) → 22.D / 22.E / 22.H / 22.C as canvas-driven needs surface. **Phase F (Designer Noticed) is satisfied by Phase 21** — do not duplicate; the spec's five-category re-skin is deferred until proposal volume motivates grouping. **Cut from v1 entirely: Linear integration** (interop ≠ moat — revisit only on explicit user signal). 22.A promotion gates on the v0.1.0 dogfood build auto-delivering reliably.
...
- [x] **F4 test reuse** — exposed `core_local::tests::boot_with_helper_status` (and `boot_with_local_ops` variant) as `pub(crate)`; new `apps/desktop/src-tauri/src/test_support.rs` hosts `CountingOps`. F4 test shrunk ~83 LOC. PR [#32](https://github.com/byamron/designer/pull/32). `CountingHandler` was *not* moved — lives in `crates/designer-claude/`, single use, no actual duplication.
