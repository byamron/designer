...
**Next: Phase 13.H — gates real-Claude usability.** PR 20 merged 2026-04-26. Five items, **one workspace, sequential** (~2–3 days): F1 wire `permission_handler.decide()` into the stdio reader (hard blocker — without it, real Claude tool use hangs), F2 populate `PermissionRequest::workspace_id` (one line, folds into F1's PR), F5 tool-use translator emitting `ArtifactProduced` per `tool_use` block (formerly PR 17's `TODO(13.D-followup)`), F3 subscribe `ClaudeSignal::Cost → CostTracker::record`, F4 route `core_git::check_track_status` through `append_artifact_with_summary_hook`. Original 13.H safety-enforcement work renumbered to **13.I** (pre-write gates, symlink-safe scope, risk-tiered approvals, binary pinning, HMAC tamper-evidence) — see `security.md`; continues to gate GA.

After 13.H ships the app is genuinely runnable end-to-end with real Claude Code, and the next phase pivots from infrastructure to dogfood-driven priorities (Phase 14 sync vs. Phase 15 polish, decided by what real workflow friction surfaces).
...
### Phase 13.H — Phase 13 hardening pass *(P0 — gates real-Claude usability)*

**Until F1+F2 land, real-Claude usage stalls on the first tool prompt.** F3/F4/F5 are functional gaps but not stalls. Detail and per-file change list in `roadmap.md` Track 13.H.

**Parallelization decision: ONE workspace, sequential.** ~500 LOC of cohesive runtime work; three of five items live in the same two files (`claude_code.rs` + `stream.rs`); the integration cost of separate parallel branches would exceed the time savings. Recommended sequence: F1 → F2 (one-line plumbing) → F5 → F3 → F4. Estimated 2–3 days end-to-end.

- [ ] **F1 — Wire `permission_handler.decide()` into the stdio reader.** *(hard blocker, ~1.5 days)* Add `TranslatorOutput::PermissionPrompt` variant + Claude prompt-JSON parse arm in `stream.rs`; in `claude_code.rs::reader_task`, route the variant to `permission_handler.decide(req)` and write `{"behavior":"allow"|"deny"}` through `stdin_tx`. Without this, real-Claude tool use hangs the agent until Claude's ~10-min internal timeout.
- [ ] **F2 — Populate `PermissionRequest::workspace_id`.** *(folds into F1's PR, 5 min)* One line at the F1 construction site. Without F2, the inbox handler fail-closes on every prompt with `MISSING_WORKSPACE_REASON`.
- [ ] **F5 — Tool-use translator + `ArtifactProduced` emission.** *(folds into F1's PR, ~half day)* Formerly the PR 17 review's `TODO(13.D-followup)`. Today `ClaudeStreamTranslator::extract_assistant_text` drops `tool_use` blocks; the user sees Claude's narration but never which tool was invoked. Emit one `ArtifactProduced { kind: Report, title: "Used <tool>", … }` per `tool_use` block. Stretch: correlate `tool_use_id` → eventual `tool_result` and emit `ArtifactUpdated`.
- [ ] **F3 — Subscribe `ClaudeSignal::Cost` to `CostTracker::record`.** *(independent file, ~half day)* `AppCore::boot` spawns a `tokio::task` that consumes `orchestrator.subscribe_signals()` and routes Cost variants to `cost.record(...)` + persists `CostRecorded`. Without this, cost chip reads `$0.00` and the cap check silently allows over-spend.
- [ ] **F4 — Route `core_git::check_track_status` through `append_artifact_with_summary_hook`.** *(independent file, ~half day)* Replace the direct `store.append` in `check_track_status` with the seam call. Without this, code-change rail summaries show raw `+12 −3 across 2 files` instead of on-device LLM lines.
