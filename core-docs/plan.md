...
**Next: Phase 13.H** — wire `permission_handler.decide()` into the stdio reader (F1), populate `PermissionRequest::workspace_id` (F2), subscribe `ClaudeSignal::Cost → CostTracker::record` (F3), route `core_git::check_track_status` through `append_artifact_with_summary_hook` (F4). Original 13.H safety-enforcement work (pre-write gates, symlink-safe scope, risk-tiered approvals, etc.) is captured separately in `security.md` and continues to gate GA.
...
### Phase 13.H — Phase 13 hardening pass *(P0 follow-ups; blocks GA per `security.md`)*

Wire the four production gaps that the integration review identified. None block the parallel-track integration merge (the mock-orchestrator path works fully and all gates are green), but the production runtime is incomplete until they land.

- [ ] **F1 — Wire `permission_handler.decide()` into the stdio reader.** `crates/designer-claude/src/claude_code.rs`'s reader task does not yet call `decide()` for Claude's `--permission-prompt-tool stdio` prompts. The `InboxPermissionHandler` is installed but never invoked under the real orchestrator; agents will hang on the first non-trivial tool prompt until Claude internally times out. Add `TranslatorOutput::PermissionPrompt`, parse Claude's stdio prompt JSON in `crates/designer-claude/src/stream.rs`, call `permission_handler.decide(req)` from the reader, and write the response back via the writer task's `stdin_tx`.
- [ ] **F2 — Populate `PermissionRequest::workspace_id`.** The inbox handler fail-closes when `workspace_id == None` with `MISSING_WORKSPACE_REASON`. When F1 lands, the wiring must construct `PermissionRequest { workspace_id: Some(spec.workspace_id), … }` so prompts route to the right workspace stream rather than auto-denying.
- [ ] **F3 — Subscribe `ClaudeSignal::Cost` to `CostTracker::record`.** `crates/designer-claude/src/claude_code.rs` broadcasts cost signals on every `result/success` line into a channel with no subscriber. Cost chip will read `$0.00` until `AppCore::boot` spawns a signal subscriber that calls `cost.record(...)` and persists `CostRecorded`.
- [ ] **F4 — Route `core_git::check_track_status` through `append_artifact_with_summary_hook`.** PR 18's seam contract mandates that every `code-change` emitter uses the hook for write-time summaries; PR 16 still emits via direct `store.append`. Until this is fixed, code-change artifact summaries on the rail will show the raw `+12 −3 across 2 files` string instead of the on-device LLM line.

#### Recommended automated test additions (folded into 13.H):

- **F1 stdio-reader test** — synthetic Claude stdout fixture that emits a `tool_use_request` permission prompt; assert `permission_handler.decide` is called once with the expected `tool` and `input`, and the writer task's stdin receives the encoded response.
- **F2 round-trip test** — `claude_code::tests` builds a `PermissionRequest` from a real Claude prompt JSON; assert `workspace_id` is `Some(ws)`.
