...
- **13.E — Track primitive + git wire** *(shipped 2026-04-25)*: `Track` aggregate + state machine, `GitOps::init_worktree` / `commit_seed_docs` / `current_status`, `core-docs/*.md` seeding, "Request merge" → `gh pr create`, repo-link modal in onboarding + Settings, edit-batch coalescer emitting `code-change` artifacts per unique diff signature, PR-open emitting `pr` artifacts.
