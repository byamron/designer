...
## Addendum (2026-04-27): Track 13.L envelope-version bump (1 → 2)

Track 13.L drops the `gh gist` + `gh issue` filer that 13.K added and reworks the Friction triage view as a master list with an explicit `Open` / `Addressed` / `Resolved` state machine. As part of that rework, the `FrictionLinked { friction_id, github_issue_url }` payload is repurposed into `FrictionAddressed { friction_id, pr_url: Option<String> }` — a non-additive change that violates clauses (a) and (d) of the previous addendum's "additive only" rule. To keep the freeze meaningful, 13.L bumps `EventEnvelope.version` from `1` to `2`: new records are written at version 2 with the new variant name, while old records (`version: 1`, `kind: friction_linked`) continue decoding through the legacy `FrictionLinked` variant and the `core_friction::project_friction` projector maps them to `Addressed { pr_url: None }`. `FrictionFileFailed` is similarly retained for legacy decode but loses its producer; a future external-filing path can re-emit it (the variant is `#[deprecated]`-tagged in the source). 13.L also adds an additive `FrictionReopened { friction_id }` variant for the master list's "Reopen" action. This is the only non-additive payload change in 13.L; subsequent tracks must continue to follow the additive-only rule above unless a new ADR addendum carves out another exception.
