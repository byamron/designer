...
- `core-docs/design-system/component-manifest.json` was invalid JSON (duplicated trailing fields after `WorkspaceStatusIcon`). Repaired; added 5 new entries (`WorkspaceThread`, `BlockRenderers`, `ComposeDock`, `SettingsPage`, `SurfaceDevPanel`); marked 5 deleted tabs as `retired`.
...
- trigger: manual (sidebar IA cleanup; the prior collapsible Archived (N) section at the bottom of the sidebar was a discoverable-but-second-class surface — promoting it gives it room to grow while shrinking the sidebar to one concern: active workspace navigation)
- archetype-reused: none (extends the existing `.sidebar-home` button class as the canonical "top-level project view tab" affordance and the existing TabLayout wrapper for the new main-pane page)
- components-reused: WorkspaceSidebar (Archived button rendered with the same .sidebar-home styling as Home — Archive icon, count badge, data-active hook); MainView (no-workspace branch now switches between HomeTabA and ArchivedView based on activeView); TabLayout; IconButton; lucide-react Archive / RotateCcw / Trash2 (all already imported elsewhere)
- components-new: ArchivedView — project-level main-pane page that lists archived workspaces in the active project with Restore + Delete IconButton actions (window.confirm gated on Delete to mirror the prior in-sidebar pattern). Internal layout: header (icon + title + subtitle) + list of `.archived-view__row` (1fr auto auto grid: name + base_branch in mono caption + actions cluster).
- primitives: none
- tokens: --space-1, --space-2, --space-3, --icon-lg, --type-h3-size, --type-h3-leading, --type-body-size, --type-caption-size, --type-family-mono, --weight-semibold, --weight-medium, --color-foreground, --color-muted, --color-border, --border-thin (all pre-existing). Archive icon uses size=16 (--icon-lg's px equivalent) in both the sidebar tab and the page header so the icon scale stays inside the canonical 12/14/16 step set defined in tokens.css.
- invariants: 6/6 pass on packages/app/src/styles/home.css (the new .archived-view__* block) and packages/app/src/styles/sidebar.css (dead-code removal). The new TSX (ArchivedView.tsx) ships no raw hex / px / ms.
- deviations: `.sidebar-home` class name now serves both the Home tab and the Archived tab. The class predates the promotion and was named for the single sidebar-Home button that existed before; it now reads as "top-level sidebar view tab." Rename deferred — the cost (two callsites + one CSS rule + a rename ripple in tests) outweighs the clarity win. The store gained `activeView: "home" | "archived"` plus `selectHomeView()` / `selectArchivedView()` actions; `selectWorkspace(null)` and `selectProject(...)` both reset activeView to "home" so closing a workspace lands the user on the canonical zero-state, not a stale Archived view from earlier in the session. activeView is intentionally **not** persisted — it's transient UI state, the same class as `quickSwitcherOpen` or `frictionMode`.
- feedback: closes the user request to make Archived feel like a first-class destination. The collapsible bottom-of-sidebar `.sidebar-group--archived` group + its `ArchivedWorkspaceRow` component were removed wholesale (per FB user-explicit-undo-shipped-work: subtraction passes that yield simpler code are accepted). Tests pass 195/195 unchanged; tabs.test.tsx beforeEach reset extended to clear activeView so future "land on Archived" tests don't leak state across the suite.

## 2026-05-03T19:00:00Z — manual (Phase 22.B: Recent Reports Home-tab surface)

- prompt: "Implement Phase 22.B — Recent Reports redesign per core-docs/roadmap.md lines 1879–1901. Lead Home-tab surface: curated highlights of shipped work, manager voice, two-step disclosure."
- trigger: manual (Phase 22.B; roadmap §22.B locks the spec — single high-level summary field, source classification, read-state projection, two-step disclosure, no project-level Reports tab per Decision 56)
- archetype-reused: none (extends the existing `.home-a__section` register so the surface sits alongside Active workspaces / Repository / Designer noticed without inventing a parallel chrome)
- components-reused: HomeTabA (Home tab now slots `RecentReportsSection` above `DesignerNoticedHome`, behind the `show_recent_reports_v2` feature flag); SettingsPage Preferences (adds a `RecentReportsToggle` mirroring `ModelsSectionToggle` / `SpineAllArtifactsToggle`); IpcClient (three new methods: `listRecentReports`, `getReportsUnreadCount`, `markReportsRead`)
- components-new: RecentReportsSection — Phase 22.B Home-tab surface. Two-step disclosure: inline `summary_high` + classification chip + workspace label + PR link is always visible; click expands the report inline with the full body and an `Open in tab` button. Three reports default; `Show more` reveals 4–5; `Show all (N)` expands the rest in place. Header reads `N unread` / `All caught up`; reads advance implicitly on inline expand and tab open, plus an explicit `Mark all read` footer.
- primitives: none. Composition with `home-a__section` + `home-a__section-head` + `home-a__link-btn` keeps the section in lockstep with sibling Home-tab sections.
- tokens: --space-2, --space-3, --space-4, --color-foreground, --color-muted, --color-border, --color-surface-flat, --border-thin, --type-body-size, --type-caption-size, --type-caption-leading, --type-family-mono, --weight-medium, --radius-badge, --radius-card, --radius-pill, --focus-outline-color, --focus-outline-width, --focus-outline-offset, --success-7, --success-11, --warning-7, --warning-11, --danger-7, --danger-11 (all pre-existing). The classification chip reuses the same semantic palette as state-dot / friction-state so feature/fix/improvement/reverted reads as the same color encoding the user already knows.
- invariants: 6/6 pass on RecentReportsSection.tsx and recent-reports.css. No raw px / hex / ms / z-index introduced.
- deviations: none. Read-state lives as a projection (single `Timestamp` per project on `Projector`, persisted in `Settings.report_read_at_by_project` outside the event log per roadmap §22.B "projection, not events"); on-disk schema extends to `(UserId, ProjectId)` keys when team-tier lands without an event-log rewrite. Classification falls back to `Improvement` for pre-22.B reports so the chip never crashes; `summary_high` falls back to `summary` for the same reason. The artifact + event-payload extensions (`summary_high: Option<String>`, `classification: Option<ReportClassification>` on `Artifact` / `ArtifactCreated` / `ArtifactUpdated`) are additive with `serde(default, skip_serializing_if = "Option::is_none")` so legacy events round-trip cleanly.
- feedback: pending. Backend tests `crates/designer-core/src/projection.rs::recent_reports_tests` cover (1) recent_reports surfaces only project reports newest-first, (2) unread count advances on mark_reports_read, (3) mark_reports_read is monotonic so a stale call can't unmark, (4) classify_from_title heuristic handles feat/fix/revert/recap. `cargo test --workspace` green; `cargo clippy --workspace --all-targets -- -D warnings` clean; `cargo fmt --check` clean. Frontend `npm run typecheck` + `npm run test` (195 tests) green.

## 2026-05-03T19:00:00Z — manual (Friction triage redesign: agent-driven loop + per-row collapse)

- prompt: "We need a better way to manage friction reports — they are piling up and I need the agent to be able to decide if they are addressed or not. … Let's start with the layout collapse first, but let's make it more agent-driven. I want to focus on making the reports, and I should be able to trust that the agent will aggregate them, plan out solutions, and solve them efficiently. Long term, this should be fully autonomous."
- trigger: manual (friction-triage redesign; staff-perspective review on the plan caught four real blockers and resolved them before implementation — see Reviewer notes in PR body)
- archetype-reused: none (extends the existing `friction-triage` list chrome)
- components-reused: SegmentedToggle (filter chips, count change), IconButton (⋯ menu trigger), MoreHorizontal lucide icon (already a lucide consumer; new icon import); CopyBatchPromptButton (renamed visual register to primary CTA, retuned label to `Triage with agent`); useCopyWithFeedback hook (now powers two paths inside FrictionRowMenu)
- components-new: FrictionTriageSection (manifest entry — was absent before; behaviour rewritten this PR), FrictionCluster (anchor-grouped wrapper with optional header — single-row clusters render flat), FrictionRow (rewritten: title + state + ⋯, agent indicator on addressed rows), FrictionRowMenu (hand-rolled popover; ESC + click-outside dismissal; auto-focuses first menuitem on open; pattern-log entry covers the no-Mini-Menu-primitive-yet rationale), humanizeAnchor (pure helper exported for tests; PascalCase → spaced, route → breadcrumb, prefixed shapes pass through)
- primitives: none
- tokens: --space-1, --space-2, --space-3, --space-4, --color-foreground, --color-muted, --color-surface-raised, --color-surface-overlay, --color-border, --accent-7, --accent-9, --accent-11, --gray-9, --border-thin, --radius-button, --radius-card, --type-body-size, --type-caption-size, --type-caption-tracking, --type-family-mono, --weight-medium, --motion-interactive, --motion-quick, --motion-emphasized, --ease-out-enter, --elevation-overlay, --focus-outline-width, --focus-outline-color, --focus-outline-offset, --layer-overlay (all pre-existing)
- invariants: 6/6 pass on packages/app/src/layout/SettingsPage.tsx + packages/app/src/styles/friction.css + packages/app/src/test/friction.test.tsx
- deviations: AddressFrictionDialog removed entirely (was the manual `Mark addressed` modal). Its only consumer was FrictionTriageSection, and the new agent-CLI loop owns that transition exclusively. Dropped the "Addressed" filter chip — addressed rows fold under "Open" with an agent indicator so the user can see "the agent is on it" without switching chips. The previous strict-equality filter would have *hidden* addressed rows from Open and made the agent loop look broken; this PR explicitly changes Open's filter to `state ∈ {open, addressed}`. Considered a context-aware empty-Open copy ("all caught up but the agent has work in flight") — found unreachable in practice (if addressed > 0, the rows are visible under Open) and removed.
- feedback: addresses the user's "reports are piling up" + "make it agent-driven" + "layout is bad because contents are squished" friction. Three-perspective plan review (staff engineer / staff UX designer / staff design engineer) caught: (1) the silent Open→Addressed fold needed a visible agent marker on the row; (2) `anchor_descriptor` is a developer-shaped string that needed a humanization pass before a manager would read the cluster header; (3) no Mini Menu primitive exists yet — chose hand-rolled popover with a pattern-log entry rather than introducing a new archetype on speculation. Tests added: Open includes addressed rows (agent-trust signal), addressed row surfaces `agent · …` indicator, multi-anchor reports cluster under a humanized header while single-anchor reports render flat, ⋯ menu shows state-conditional items, humanizeAnchor handles PascalCase / route / prefixed-shape / empty-string inputs. 200/200 frontend tests green; cargo workspace tests green; clippy clean.

## 2026-05-03T20:30:00Z — manual (Friction triage UX polish: relative-time meta)

- prompt: "add the ux polish and update the PR" — actioning the deferred UX-review item that flagged `toLocaleString()` as too verbose for the meta line at the 960×640 minimum window.
- trigger: manual (post-PR review polish; arrow-key menu nav + addressed-copy variants are still deferred per the reviewer's "wait for second consumer / dogfood signal" guidance)
- archetype-reused: none
- components-reused: FrictionRow (meta-line render only)
- components-new: `formatRelativeTime` (pure helper, exported for tests; buckets: <1min "just now" / <60min "{n}m ago" / <24h "{n}h ago" / <7d "{n}d ago" / past-week → compact "Mon DD" with year suffix only when the entry is not in the current calendar year; future timestamps clamp to "just now" so clock skew never renders a negative duration)
- primitives: none
- tokens: unchanged (no new CSS — meta line uses the existing `--type-caption-size` register)
- invariants: 6/6 pass on packages/app/src/layout/SettingsPage.tsx + packages/app/src/test/friction.test.tsx
- deviations: none. Considered using `Intl.RelativeTimeFormat` — rejected as overkill for five buckets, and the localized strings for "just now" / "h ago" don't compose into a single `format()` call without an explicit unit cascade. The hand-rolled bucket fits in 18 lines and stays trivially testable.
- feedback: closes the UX-review FOLLOW-UP on `created_at` verbosity. Full timestamp moved to the `title` attribute on the meta span so a hover discloses the exact local-format date/time. New test (`formatRelativeTime renders compact buckets…`) locks the bucket boundaries + the cross-year + clock-skew edge cases. 18/18 friction tests green; tsc clean.

## 2026-05-03T21:50:00Z — manual (Phase 22.G: team identity color tokens)

- prompt: "Implement Phase 22.G — Color system (axiom #3 amendment) per core-docs/roadmap.md lines 1852–1875. Tokens-only work; no UI consumes the new tokens yet."
- trigger: manual (axiom amendment + token addition; no UI generation skill fired — the only React surface is a dev-only lab catalog for visual review)
- archetype-reused: none. The team-dot rendering convention follows the precedent set by `.state-dot` in `packages/app/src/styles/sidebar.css` (same dot size via `--space-2`, same pulse-amplitude shape with opacity 1 ↔ 0.55, same `prefers-reduced-motion` fallback).
- components-reused: none
- components-new: TeamColorCatalog (lab-only review surface). Inline-style composition; not refactored to Mini primitives because lab pages are dev surfaces only and existing siblings (ComponentCatalog, AnnotationLayer, VariantExplorer, PrototypePreview) follow the same convention.
- components-removed: none
- css-new: `packages/ui/styles/team-pulse.css` — sixteen `@keyframes team-pulse-N` rules + `.team-dot` / `.team-dot--N` utility classes that bind hue (`--team-N`) and rate (`--team-pulse-rate-N`) by index. Honors `prefers-reduced-motion` with `animation: none; opacity: 1`.
- css-modified: `packages/ui/styles/tokens.css` — added `--team-1..--team-16` (light + dark anchors via `.dark-theme`), `--team-1-tint..--team-16-tint` derivations using `color-mix(in srgb, ...)` against `--team-tint-light` (7%) / `--team-tint-dark` (10%) alphas, and `--team-pulse-rate-1..-16` baked at 1.413–1.991 s.
- tokens-new: `--team-1..--team-16`, `--team-1-tint..--team-16-tint`, `--team-tint-light`, `--team-tint-dark`, `--team-pulse-rate-1..-16`. Sixteen OKLCH hues 22.5° apart, fixed chroma 0.10 (deliberately below Radix semantic-9 chroma so signal colors still pop). Light-mode L=0.55, dark-mode L=0.78.
- tokens-modified: none. The amendment is purely additive; no existing accent / semantic / surface tokens were rebound or removed. T-G-4 (no regressions in existing accent/semantic tokens) holds.
- axioms: **#3 amended.** Three orthogonal color axes — accent (monochrome), identity (team-N, chromatic), semantic (signal, chromatic). `design-language.md` change-log dated 2026-05-03 carries the amendment text; `pattern-log.md` entry (2026-05-03 — Team identity vs accent vs semantic) explains the three-axis discipline and why team chroma is locked below the semantic palette.
- invariants: 6/6 pass on `packages/ui/styles/team-pulse.css`, `packages/ui/styles/tokens.css` (exempt), and `packages/app/src/lab/TeamColorCatalog.tsx`. `tools/manifest/check.mjs` failed on the first push (missing manifest entry for the new lab page) and was fixed in a follow-up commit; `core-docs/design-system/component-manifest.json` now carries the TeamColorCatalog entry.
- typecheck: clean (`tsc --noEmit` 0 errors).
- tests: 195/195 frontend (vitest) pass. Rust workspace clippy + fmt clean; one pre-existing flaky timing test (`coalescer_attributes_reply_to_tab_at_first_recv_not_at_flush`) fails on first run and passes on retry — unrelated to this PR (zero Rust files touched).
- deviations: (1) The lab page `TeamColorCatalog.tsx` uses inline `CSSProperties` styles instead of composing Mini primitives (Box / Stack / Cluster). Acceptable per the established lab-page convention; if any of the surface graduates to production, refactor at consumption time. (2) Sixteen `@keyframes team-pulse-N` rules carry identical bodies — kept distinct per the roadmap spec ("16 named keyframe rules — do not generate at runtime") so the per-team rate is bound at the rule level, not via runtime CSS-var injection. (3) `color-mix(in srgb, var(--team-N) X%, transparent)` is used for tint derivations rather than `in oklch`; sRGB is the correct space for alpha blending against transparent (oklch mixing toward `transparent` would interpolate L/C/H toward zero, darkening the tint).
- feedback: pending. Follow-ups parked for Phase 22.A consumption — light-mode dot/tint amplitude tuning if visibility on white surfaces feels weak in dogfood; potential gamut-clipping audit for blue/cyan/violet hues at L=0.55 C=0.10 if some hues read as desaturated grey on first canvas render. The tokens are tunable from one place; consumers can re-anchor without changing call sites.

## 2026-05-04T00:55:00Z — manual (compact braille spinner + per-tab activity isolation)

- prompt: "the three dot loading animation is clunky and too big and too wide. Use a more compact unicode animation like conductor (it can be randomized, or the dots in the animation can follow a path. we don't want to copy them, just be inspired by them)" + "i think the 'Designer is thinking' indicator is possibly mistakenly persisting across tabs"
- trigger: manual (chat-reliability dogfood pass; replaces ActivityIndicator dot-row markup, threads per-tab state through WorkspaceThread)
- archetype-reused: none (extends `.thread__activity` chrome introduced in B7)
- components-reused: ActivityIndicator (collapsed JSX from three child spans to a single `.thread__activity-spinner` span; CSS-driven `::before { content }` keyframe replaces the JS-free three-pulse animation)
- components-new: none — the spinner is a CSS variation of the existing indicator
- primitives: none
- tokens: --motion-spin (NEW: `800ms`; full cycle for compact unicode spinners; sits next to --motion-pulse `1200ms` and --motion-blink `700ms` in tokens.css), --color-foreground, --color-muted, --type-family-mono (all pre-existing)
- invariants: 6/6 pass on packages/app/src/styles/blocks.css (`.thread__activity-spinner` block) + packages/ui/styles/tokens.css (`--motion-spin` literal `800ms` declared in the project-level `--motion-*` group, identical shape to the existing tokens). No raw `px` / `hex` / `rgba(` / `z-index` literals introduced.
- deviations: opacity literals `0.7` (submitting) and `0.5` (stuck) on `.thread__activity-spinner` are intentional — they're styling (de-emphasis register), not signal. Promoting to a token would require a design-language amendment and the values are defensible in light + dark per the staff design-engineer review. Reduced-motion fallback uses `…` (U+2026 horizontal ellipsis) rather than the all-dots braille `⠿` — `⠿` was a wash against the spinner's individual rotation frames at a glance, making "thinking" visually indistinguishable from a frozen frame for reduced-motion users (UX review BLOCKER, fixed before merge).
- feedback: pending. New regression test `chat-states.test.tsx > 'activity state is per-tab — a stalled send on A does not bleed into B'` covers the per-tab isolation invariant: stalled `postMessage` on tab A → switch to tab B → `data-component="ActivityIndicator"` is null on B and `form.compose` has no `aria-busy`; switch back to A → indicator returns. 196/196 frontend tests pass. Pattern-log entry added below for the spinner glyph palette + reduced-motion choice + cross-surface motion duration rationale.


## 2026-05-03T22:20:00Z — manual (Phase 22.I: track shipping history badge + PrOpen→Merged crossfade)

- prompt: "Implement Phase 22.I — Track completion + shipping history per core-docs/roadmap.md lines 1963–1976. The payoff phase for the canvas: when a real track ships, the node lights up with a green check and a 'Shipped here' badge that persists permanently."
- trigger: manual (Mini procedure followed by hand — `generate-ui` did not fire because the surface is a small append to `RoadmapCanvas`, not a new screen)
- archetype-reused: none (the persistent badge follows the multi-claim-stack grammar: stack-with-overflow-tooltip, three pills then `+N`, mirrors `RoadmapMultiClaimStack` so the live and historical surfaces read as parallel artifacts)
- components-reused: Tooltip (audit-trail body), RoadmapStatusCircle (extended for the crossfade), RoadmapCanvas (one new sibling line under the headline)
- components-new: ShippedHereBadge (`packages/app/src/blocks/ShippedHereBadge.tsx`)
- components-removed: none
- css-new: `.roadmap-shipped-here-badge` family (pill + overflow chip + focus-visible outline) and `.roadmap-node-row__shipments` (the row's secondary line, indented under the headline). Status-circle gains `.roadmap-status-circle__done-overlay` opacity-transition rule for the crossfade.
- css-modified: `roadmap.css` — extended the existing `prefers-reduced-motion` block to drop the new opacity transition; existing chevron + arc transitions were already correctly gated.
- tokens-new: none.
- tokens-referenced: --space-1, --type-caption-size, --weight-semibold, --color-muted, --color-surface-overlay, --color-border-soft, --border-thin, --radius-2, --target-sm, --focus-outline-color, --focus-outline-width (badge); --motion-emphasized + --ease-out-enter (crossfade); --success-9 (Done fill); --color-surface-base (checkmark stroke). All pre-existing.
- invariants: 6/6 pass on packages/app/src/blocks/ShippedHereBadge.tsx + packages/app/src/components/RoadmapCanvas.tsx + packages/app/src/components/RoadmapStatusCircle.tsx + packages/app/src/styles/roadmap.css.
- typecheck: clean (`tsc --noEmit` 0 errors).
- tests: 208/208 frontend (vitest) pass — six new in `shipped-here-badge.test.tsx` covering empty input, audit-trail copy, overflow `+N` collapse, and team-label override. 50/50 designer-core lib tests pass (two new lifecycle fixtures: shipment persists across archive, shipment replay is idempotent on `(node_id, track_id)`). 22/22 core_git tests pass (three new: `complete_track` emits the shipment when anchored, skips it when unanchored, and is idempotent against double-fire).
- deviations: (1) `PullRequestOpened` event gained an additive `pr_url` field with `serde(default)` for legacy decode — followed the same Lane 0 ADR shape that 22.A used for `TrackStarted.anchor_node_id`. Locked by a new `legacy_pull_request_opened_without_url_decodes` test. (2) Live-claim cleanup migrated from "only on `TrackArchived`" to "either `NodeShipmentRecorded` or `TrackArchived`, whichever fires first" — the archive arm stays as an idempotent fallback so a track that's archived without merging still cleans up. (3) The Done overlay is always rendered into the SVG with `opacity: 0` by default; previously the JSX gated it on `isShipped`. The opacity layering is the only way to get a smooth opacity transition through React's render — toggling DOM presence breaks the crossfade because there's no `from` state in CSS.
- feedback: pending. The "demonstrably alive" moment (status circle's PrOpen → Merged green-flip) needs a real track merge to dogfood end-to-end; the unit-test coverage locks the projection contract but the manual quality gate (capture before/after screenshot in PR description) is the trust layer.


## 2026-05-05T04:41:49Z — manual (ADR 0009 + roadmap restructure: Build/Harden alternation + parking lot)

- prompt: "Run /staff-perspective-review on this plan, optimize it, then draft the final implementation" — restructure the active roadmap around a fifth product principle ("Shipped state is trustworthy") with Build/Harden alternation and a parking-lot mechanism that defers (does not delete) ambition.
- trigger: manual (process change with documentation impact; not a `generate-ui` firing — no UI generated. Logged because ADR 0009 binds future Harden phases to a verification mechanism that will produce UI artifacts: golden-path screencasts checked in per release tag and bound to a Playwright test in Phase 26H.)
- archetype-reused: none (process / docs change)
- components-reused: none
- components-new: none
- components-removed: none
- css-new: none
- css-modified: none
- tokens-new: none
- tokens-referenced: none
- invariants: N/A (no UI files touched). `tools/invariants/check.mjs` not applicable.
- typecheck: N/A (no TS/Rust changes).
- tests: N/A (no code changes; all gates remain green from the Phase 24 step-1 baseline).
- files: `core-docs/architecture/adr/0009-trustworthy-shipping.md` (new), `core-docs/parking-lot.md` (new), `CLAUDE.md` (added fifth principle + fifth Quality Bar item + Build/Harden note in How to Work + parking-lot row in Core Documents table), `core-docs/architecture/spec.md` (Decision 64 added to Decisions Log), `core-docs/roadmap.md` (active sequence callout at top; Phase 24 / 24H / 24I / 25 / 25H / 26 / 26H detail sections inserted before Milestones; Parked callouts added at Phase 15.H / 17 / 18 / 19 / 20 / 21.A2 detector squad / 22 headers), `core-docs/plan.md` (rewritten 517 LOC → 42 LOC).
- deviations: (1) Three-perspective staff review ran against the strategic plan, not a code diff — adapted the skill mid-flight per its own "if a perspective genuinely doesn't apply, say so" guidance. Findings + triage saved to `.context/roadmap-restructure/plan-v2.md`. (2) ADR 0009's verification mechanism (golden-path screencast + Playwright binding) lands in Phase 26H, not in this PR — the principle and structure ship now; the gating implementation follows. (3) v1 plan's "subtract `core_*/commands_*` split" reclassified to parking lot per engineering review — the convention's cognitive cost is real but reverting it across 5+ files is a multi-PR refactor, not a Harden-phase subtraction. (4) Designer-Noticed first-detector pick deferred to Phase 26 start (default candidate `repeated_correction`, final pick gated on dogfood evidence) per UX review — confidence-rank ≠ user value. (5) macOS Playwright CI runner parked rather than shipped in 26H per design-engineering cost analysis ($28–42/mo + setup friction); Linux Playwright + manual macOS spot-check is the v1 verification path.
- feedback: pending. The structural change is reversible (parked phases re-activate by friction trigger; the principle can be amended via a successor ADR); first signal of fit lands when Phase 24H's friction-closure pass runs against the chat-v2 inbox.


## 2026-05-07T05:17:53Z — manual (Phase 24 §5.4: send-while-streaming queue + stop-and-send)

- prompt: "Phase 24 §5.4 — send-while-streaming queue (default ⏎) + stop-and-send (⌘⏎). Hover/focus menu on the Send button surfaces both modes; no chevron, no split-button. Queue persists per-tab to localStorage and auto-dispatches on the working→idle transition."
- trigger: manual (Mini procedure followed by hand — `generate-ui` did not fire because the surface is a small extension to ComposeDock plus a new SendMenu primitive, not a new screen)
- archetype-reused: Tooltip-pattern surface (cloneElement trigger + fixed-position floating layer + two-pass measure→clamp), reapplied for `role=menu` instead of `role=tooltip`
- components-reused: ComposeDock (extended), ComposeDockActivityRow (read for subprocess state), IconButton, IconX
- components-new: SendMenu (`packages/app/src/components/SendMenu.tsx`)
- components-removed: none
- css-new: `.compose__queued` family (chip + prefix + text + cancel) and `.send-menu` family (surface + items + labels + kbd) in `packages/app/src/styles/chat.css`
- css-modified: chat.css extended; no existing rules changed
- tokens-new: none
- tokens-referenced: --space-1, --space-2, --space-3, --space-4, --radius-button, --radius-badge, --border-thin, --color-foreground, --color-muted, --color-border, --color-border-soft, --color-surface-raised, --color-surface-sunken, --color-surface-muted (with --color-surface-sunken fallback), --color-surface-hover (with --color-surface-raised fallback), --color-foreground-muted (with --color-muted fallback), --elevation-overlay (with rgba shadow fallback), --type-body-size, --type-caption-size, --type-caption-leading, --type-family-mono, --weight-medium, --motion-quick, --focus-outline-width, --focus-outline-color, --layer-overlay (with 1000 fallback)
- invariants: 6/6 pass on packages/app/src/components/SendMenu.tsx + packages/app/src/components/ComposeDock.tsx + packages/app/src/styles/chat.css
- typecheck: clean (`tsc --noEmit` 0 errors)
- tests: 236/236 frontend (vitest) pass — eleven new in `compose-queue.test.tsx` covering ⏎-queues-when-running, ⏎-sends-when-idle, ⌘⏎-queues-and-interrupts, ⌘⏎-falls-back-to-send-when-idle, queue-chip-renders, cancel-button-clears, SendMenu-only-mounts-when-running, Send-button-label-flips-to-Queue-when-running, setQueuedMessage-trims-and-persists, empty-string-deletes-and-purges-localStorage, clearQueuedMessage-idempotent.
- files: `core-docs/phases/phase-24-pass-through-chat.md` (§5.4 spec rewrite — queue + stop-and-send + hover menu UX + visual treatment + flag), `packages/app/src/store/app.ts` (queuedMessageByTab + setQueuedMessage / clearQueuedMessage + localStorage persistence + clearTabState reaping), `packages/app/src/components/SendMenu.tsx` (NEW), `packages/app/src/components/ComposeDock.tsx` (subprocess-state read + Enter pivot + ⌘⏎ stop-and-send + queue chip render + SendMenu mount when running), `packages/app/src/tabs/WorkspaceThread.tsx` (auto-dispatch effect on working→idle transition + queue clear), `packages/app/src/styles/chat.css` (compose__queued + send-menu styles), `packages/app/src/test/compose-queue.test.tsx` (NEW), `core-docs/design-system/component-manifest.json` (ComposeDock entry extended for §5.4 + new SendMenu entry).
- deviations: (1) Queue stores text only for v1; attachments stay attached to the live composer until dispatch. Attachment-while-streaming is rare enough that text-only queue is the right v1 cut — documented in spec §5.4 + the queueDraft body comment. (2) Auto-dispatch fires only for the active tab. If the user switches tabs mid-stream and the inactive tab's turn ends, the queue persists but doesn't auto-fire — the user sees the chip on switch-back. Out-of-active-tab auto-dispatch needs a global watcher; deferred to a follow-up if dogfood surfaces the gap. Documented in WorkspaceThread auto-dispatch effect comment. (3) The hover menu reuses Tooltip's positioning math (cloneElement trigger + two-pass measure → clamp) rather than introducing a Mini primitive — Mini doesn't yet have a HoverMenu primitive, and the SendMenu surface is single-use enough that introducing one for one consumer would be premature. If a second consumer surfaces, extract to `packages/ui/src/primitives/HoverMenu.tsx` with the existing pattern.
- feedback: pending. The two-mode UX is an architectural commitment (queue is the must-ship default; stop-and-send is the alternate); dogfood will tell us whether the 200ms hover-reveal delay is right and whether ⌘⏎ vs ⇧⏎ is the right chord. The `show_compose_stop_and_send` feature flag from spec §5.4 is reserved as a kill-switch but is not yet wired — defaulting to ON; if the alternate flow surfaces real issues, gating goes in via a follow-up commit before flag-flipping show_chat_v2.


## 2026-05-08T04:54:31Z — manual (Phase 24 §5.4.2: SIGINT interrupt + Esc priority chain)

- prompt: "let's do 5.4.2" — replace the in-band control_request interrupt mechanism with libc::kill(pid, SIGINT) per the PR #117 spike findings, and wire the §5.4.1 Esc priority chain (composer-focused rules 4 + 5).
- trigger: manual (Mini procedure followed by hand — pure backend + frontend keybinding work, no UI generation)
- archetype-reused: none (no UI surface added)
- components-reused: ComposeDock (Esc handler extension), existing IPC client.interruptTurn
- components-new: none
- components-removed: interrupt_request_line builder + its test (in-band control_request mechanism retired)
- css-new: none
- css-modified: none
- tokens-new: none
- tokens-referenced: none (no UI surface)
- invariants: 6/6 pass on packages/app/src (no CSS / TSX changes that touch tokens)
- typecheck: clean (`tsc --noEmit` 0 errors)
- tests: 241/241 frontend (vitest) pass — three new in compose-queue.test.tsx covering Esc priority chain (rule 4: queue discard, rule 5: subprocess interrupt, no-op when neither). 49/49 Rust test suites pass with no failures.
- files: `crates/designer-claude/Cargo.toml` (libc promoted from optional dev dep to unconditional), `crates/designer-claude/src/claude_code.rs` (interrupt() rewritten to call libc::kill(child_pid, SIGINT) with errno + race-guard handling; in-band interrupt_request_line builder + test removed), `packages/app/src/components/ComposeDock.tsx` (Esc priority chain in textarea onKeyDown — rule 4 queue discard, rule 5 cmd_interrupt_turn), `packages/app/src/test/compose-queue.test.tsx` (3 new Esc tests), `core-docs/design-system/pattern-log.md` (SIGINT-vs-control_request decision rationale entry).
- deviations: (1) Spike's optional in-band fallback path ("if SIGINT is denied, fall back to control_request") rejected for v1 — SIGINT against a child the orchestrator spawned cannot fail under normal operation; the denial case is hypothetical, and the helper can be reconstructed from the spike binary if it ever surfaces. (2) No unit test for SIGINT delivery itself — would require spawning a real subprocess and asserting on its kill state, which is what the live integration test (claude_live feature, claude_live.rs) already covers via the spike's transcript shape. The unit-testable error paths (TeamNotFound, race-with-spawn) are already implicitly covered by existing tests.
- feedback: pending. The SIGINT path now reaches all of the queue + stop-and-send + Esc-while-running flows from PR #124. Dogfood will reveal whether the SIGINT round-trip latency feels right (PR #117 measured ~7s from signal to subprocess exit, dominated by API-side ack); if it's noticeable, a future polish PR could pre-emit the optimistic AgentTurnEnded { Interrupted } from the orchestrator before the actual subprocess exits.


## 2026-05-12T22:05:00Z — manual (Phase 24 §5.2: render-time activity indicator)

- prompt: "let's finish phase 24" → step 10 of the workspace sequence: wire the activity indicator as a render-time observable per spec §5.2 (subprocess running AND turn open), preserve the elapsed-time chip, add the AgentTurnEnded fade, switch the pulse color to monochrome `--color-muted`.
- trigger: manual (Mini procedure followed by hand — UI edit on a pre-existing component, not a fresh generation)
- archetype-reused: none (existing component extended)
- components-reused: ComposeDockActivityRow (signature extended with exit-fade state machine; render branch for chat-v2 reads `phase24Open` as `number | null` from `chatThreadStore`)
- components-new: none
- components-removed: none
- css-new: `.compose-dock-activity-row[data-exiting="true"]` rule (opacity 0); reduced-motion override extended to suppress the opacity transition.
- css-modified: `.compose-dock-activity-row` (opacity baseline + `var(--motion-exit)` transition); `.compose-dock-activity-row__pulse` background `--accent-9` → `--color-muted` (spec-mandated monochrome).
- tokens-new: none
- tokens-referenced: `--motion-exit`, `--color-muted` (added/reused). Spec said `--motion-fast` for the fade; resolved to the composed `--motion-exit` token (= `--motion-quick` 120 ms + `--ease-in-exit`) per the design-language exit-motion compound — see pattern-log entry. (First cut used `var(--motion-quick) ease-out`, which split the compound and used the wrong easing primitive; staff-review caught it and the post-review commit uses `var(--motion-exit)`.) The 1.2s vs 1.6s pulse-duration discrepancy (spec says `--motion-pulse` is 1.6s, the token is 1200 ms) is filed as a Phase 24H FOLLOW-UP rather than changing the global token (the same token drives the tab-strip badge; changing it here affects multiple surfaces).
- invariants: 6/6 pass on packages/app/src (no arbitrary px/hex/ms/z-index/hex-in-tsx; primitives imports clean)
- typecheck: clean (`tsc --noEmit` 0 errors)
- tests: 247/247 frontend (vitest) pass — five new in `compose-dock-activity.test.tsx` covering Phase 24 §5.2: chip shows when subprocess+turn open both true, chip hidden when only one is true (no half-state), fade on turn end with unmount after CHIP_EXIT_MS, fade cancellation when a new turn arrives mid-fade, elapsed counter reads the OPEN turn's `started_at` not `Date.now()`. Existing 10 chat-v1 tests preserved (snap-off + accent-9 awaiting-approval override unchanged).
- files: `packages/app/src/components/ComposeDockActivityRow.tsx` (selector returns primitive `number | null`; exit-fade state machine; `data-exiting` attribute), `packages/app/src/styles/chat.css` (opacity transition + monochrome pulse + reduced-motion override), `packages/app/src/test/compose-dock-activity.test.tsx` (5 new chat-v2 tests + chatThreadStore/flagsStore fixtures), `core-docs/design-system/component-manifest.json` (ComposeDockActivityRow entry updated for §5.2 contract + tokens), `core-docs/design-system/pattern-log.md` (motion-fast→motion-quick mapping + monochrome-pulse rationale).
- deviations: (1) The `phase24Open` selector previously returned a fresh object literal each render — a latent bug that escaped because no test exercised the chat-v2 branch. Switching to a primitive `number | null` stabilizes the selector's referential equality. (2) Spec said the pulse uses `--motion-pulse` at 1.6s; the project's `--motion-pulse` is 1200ms. Not changed globally because the token drives the tab-strip badge too — file a Phase 24H FOLLOW-UP to resolve the 1.2s/1.6s discrepancy with a project-wide decision rather than ad-hoc per-surface tokens. (3) Spec said `--motion-fast` for the exit fade; no such token exists. Resolved to the composed `--motion-exit` token (= `--motion-quick` + `--ease-in-exit`) per the design-language exit-motion compound. First cut split the compound (`var(--motion-quick) ease-out`); staff-review caught the easing mismatch and the post-review commit uses `var(--motion-exit)` directly. Pattern-log updated to add the rule "don't disassemble composed motion tokens into duration + raw easing." (4) Staff-review UX surfaced a spec §5.7 row 278 gap — the activity-indicator container needs a static `aria-label="Agent working"` (separate from the nested live region). Inlined the fix in the same PR; test added.
- feedback: pending. Dogfood will tell whether the 120ms fade is the right tempo and whether the monochrome pulse reads correctly across accent × mode combinations (visual baselines for the chat-v2 surface are gated on the flag flip per the Phase 24H roadmap entry).


## 2026-05-12T22:50:00Z — manual (Phase 24 §5.6: error-state copy mapping)

- prompt: "let's finish phase 24" → step 12: implement the §5.6 error-state copy table in chat-v2 (WorkspaceThread + ChatStreamRenderer error surfaces).
- trigger: manual (Mini procedure followed by hand — extension of an existing component cluster, not a fresh generation)
- archetype-reused: none
- components-reused: ChatStreamRenderer (extends `AgentTurnRow`'s stop-reason rendering), WorkspaceThread (extends the aria-live announcement effect pattern from PR #125)
- components-new: ErrorMarker, MaxTokensMarker (inline siblings of existing InterruptedMarker; both internal to ChatStreamRenderer.tsx, not exported)
- components-removed: none
- css-new: `.thread__turn-end-marker` + `.thread__turn-end-marker::before` (warning-accent pill icon) + `.thread__turn-end-marker-text`. Mirrors the interrupted-marker register; foreground uses the new `--color-foreground-muted` semantic token, accent pill uses the new `--color-warning` semantic token.
- css-modified: none (existing `.thread__interrupted-marker` unchanged)
- tokens-new: `--color-foreground-muted` (alias of `--color-muted`), `--color-warning` (alias of `--warning-9`) — both added to `packages/app/src/styles/app.css` light + dark blocks. Semantic names; spec §5.6 prose uses these names, so the role layer now resolves them directly without raw scale references in component CSS.
- tokens-referenced: `--color-foreground-muted`, `--color-warning`, `--space-2`, `--radius-pill`, `--type-caption-size`.
- invariants: 6/6 pass on packages/app/src
- typecheck: clean
- tests: 248/248 frontend (vitest) pass — render-altitude tests for `ErrorMarker` / `MaxTokensMarker` / `ErrorAnnouncement` are gated on the chat-v2 flag-flip per the Phase 24H roadmap entry that already groups `bootReplaying`, queue auto-dispatch, and `InterruptAnnouncement`. Added the new markers to that filed entry rather than diverge from the existing parking-lot policy (InterruptedMarker has no render-altitude test either; pinning ErrorMarker now would create an asymmetric coverage gap). 2 new Rust tests on `humanize_dispatch_error` pin the §5.6 row-1 ChannelClosed copy verbatim ("Couldn't reach Claude. Check your installation in Settings → Account.").
- files: `packages/app/src/blocks/ChatStreamRenderer.tsx` (ErrorMarker + MaxTokensMarker + stop_reason dispatch), `packages/app/src/tabs/WorkspaceThread.tsx` (erroredTurnIds selector + ErrorAnnouncement region), `packages/app/src/styles/blocks.css` (`.thread__turn-end-marker` cluster), `packages/app/src/styles/app.css` (semantic tokens), `apps/desktop/src-tauri/src/core_agents.rs` (humanize_dispatch_error copy + 2 tests), `core-docs/design-system/component-manifest.json` (ChatStreamRenderer tokens + last_updated), `core-docs/design-system/pattern-log.md` (semantic-tokens-for-spec-copy rule).
...
- files: `packages/app/src/home/HomeTabA.tsx` (Disconnect button wrapped in `{hasLinkedWorkspace && (...)}` instead of `disabled={!hasLinkedWorkspace}`), `core-docs/design-system/component-manifest.json` (HomeTabA purpose + last_updated).
...
- files: `packages/app/src/util/autoname.ts` (new), `packages/app/src/test/autoname.test.ts` (new, 20 tests), `packages/app/src/tabs/WorkspaceThread.tsx` (autoRenameOnFirstMessage helper, session-level autoRenamedRef guard, polite RenameAnnouncement live region), `core-docs/design-system/component-manifest.json` (WorkspaceThread last_updated + purpose).
