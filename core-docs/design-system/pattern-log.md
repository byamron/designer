...
## 2026-05-03 — Team identity vs accent vs semantic: three orthogonal color axes

Phase 22.G introduces sixteen `--team-1..--team-16` OKLCH hues for team identity on canvas surfaces (row tints on claimed nodes, status circles, team-label dots, attention-card pills). The temptation when adding any chromatic palette to a previously-monochrome system is to fold it into the existing accent slot — repurpose `--accent-*` so the same scale carries both "this is the active control" and "this is Alex's work." That collapse breaks the axiom, and we explicitly rejected it.

Designer now has **three independent color axes**, and each one means something different. Accent (`--accent-*`, bound to `--gray-*`, monochrome by axiom #3) carries *controlled emphasis* — focus rings, the energized state on a button, the active item in a list. Identity (`--team-*`) carries *whose work this is* — assigned to a team, persistent for the duration of that team's involvement, never carrying signal beyond "this thing belongs to that team." Semantic (`--success-*` / `--warning-*` / `--danger-*` / `--info-*`) carries *what state this is in* — done, blocked, errored, info. The three axes can co-occur on a single surface: a status circle (semantic green for "done") inside a row with a team tint (identity for "Alex's") and a focus ring (monochrome accent for "you're focused on this") — and each axis stays legible because none is competing for the same channel.

The chroma anchor enforces the orthogonality at the token level. Team chroma is fixed at 0.10 — deliberately below the semantic palette (Radix green-9 / amber-9 / red-9 / blue-9 sit around 0.13–0.20). When a status circle (semantic, saturated) lands inside a row with a team tint (identity, muted), the semantic still pops as the figure and the identity recedes as the ground. This is the inverse failure mode of the typical "vibrant brand color makes critical-error-red disappear" — we prevent it structurally rather than by review. Anyone tempted to bump team chroma to "make it more colorful" must amend axiom #3 first; the pattern is locked here so the conversation is grounded.

The "monochrome chrome, color on canvas" split is the second invariant. App chrome — sidebars, topbars, tab bars, modal frames, the project strip — stays bound to the gray scale. Identity color appears only on canvas surfaces where multiple agents' work is being represented in parallel (the future workspace canvas, attention-card surfaces). This is what makes the manager-of-agents metaphor read: when the user looks at the canvas, *the work itself is colored* by who's doing it; when they look at the chrome, the tooling stays out of the way. A team tint on a sidebar row would muddy that read by suggesting the chrome is part of the work. Future surfaces that want team color should ask: am I chrome or am I canvas? — and the answer is binary.

## 2026-05-03 — Phase-state region wrapper for layout-stable disclosures (ToolUseLine)

`ToolUseLine`'s expanded surface owns three states that share one footprint: `loading` (pre-IPC), `loaded` (`<pre>` with payload body), `error` (status text + retry button). Original PR #92 conditionally rendered each state as a sibling, which meant payload arrival visibly shifted the row by ~6–10px (loading `<p>` margin: 0 vs. `<pre>` with padding + border). PR #94 introduced `.tool-line__region` — a flex-column wrapper that owns the box chrome (sunken fill, soft border, paddings) and a `min-height` calc of `(--type-caption-size × --type-caption-leading) + 2*--space-2 + 2*--border-thin`, sized to one line of monospace at caption-size. Loading / loaded / error are plain content inside the wrapper; the box never shifts.

The calc has a load-bearing trap: `--type-caption-leading` is a unitless `1.4` multiplier. Adding it directly to lengths (`0.5rem + 2px`) makes CSS calc() reject the entire rule, the `min-height` silently disappears, and the layout-stability promise breaks. Multiplying leading by size (`var(--type-caption-size) * var(--type-caption-leading)`) is the only correct shape. Round-3 design-engineer review caught this — round-2 review missed it because the rule appeared to apply in jsdom (no parser strictness). Future code that calcs against a typography token must distinguish unitless (`-leading`) vs. length (`-size`) tokens explicitly.

The wrapper also owns `role="region"` + `aria-label` + `aria-live="polite"` + `aria-busy`. The inner `<pre>` is plain content — *not* a live region — because some screen readers (JAWS in particular) read the entire content of a `<pre>` verbatim when it's inserted into a live region; with 40 lines of code that's a ~15-second announcement. Moving the live-update semantics up to the wrapper means a single region-change notification fires on phase transitions, and the user navigates into the `<pre>` on demand to read its content.

## 2026-05-03 — Frontend retry sidesteps Rust-side error classification (ToolUseLine)

Phase 23.C originally specified Rust-side error classification on `getArtifact` rejection: 404 → permanent → no retry button; 5xx / IPC error → transient → retry button. PR #97 ships a frontend-only path instead: the error state always renders a "Try again" text-link beside "Nothing to show.", click clears `fetchedRef`/`inflightRef` and refires `fetchPayload()`. Cost is one extra IPC on a permanent-404 retry click (which then re-renders the same error state); transient failures recover with one click without us having to type-classify the rejection reason.

Why the frontend-only path won: classifying errors needs an enum on the Rust IPC layer (today `cmd_get_artifact` returns `Result<ArtifactDetail>` with a string-typed error), tests on both sides, and a coordinated DTO addition. The user-paid cost is a single click + IPC roundtrip on permanent failures — not silent overhead, not a footgun. If/when dogfood shows the extra IPC matters (e.g., users repeatedly retrying a known-permanent 404), the proper Rust-side classification is still the right path; tracked as roadmap item 23.C.f4. Pattern generalizes: when a UX problem reads as "we need to know which kind of error this is," consider whether letting the user choose is acceptable. Often it is, and it ships in hours instead of days.
...
## 2026-05-03 — `OrchestratorEvent::ActivityChanged` is an additive variant on a broadcast-only enum (Phase 23.B)

`OrchestratorEvent` is the broadcast-only normalized stream emitted by orchestrators (`crates/designer-claude/src/orchestrator.rs`). Phase 23.B added `ActivityChanged { workspace_id, tab_id, state, since: SystemTime }` to that enum. ADR 0002 froze `EventPayload` (the persisted domain-event surface) and required an ADR addendum for any change there — but `OrchestratorEvent` is a different shape: it has no projector arm, no replay invariant, no on-disk representation. The reader-loop pattern-matches it into an `EventPayload` only for the variants that *should* be persisted; the new `ActivityChanged` arm sits in the same broadcast-only bucket as `ArtifactProduced` / `ArtifactUpdated` (which AppCore's coalescer is the single writer for) and is intentionally not persisted at all. Activity is a transient signal — by the time replay would run, the state has already changed.

Documented here so future broadcast-only additions don't re-litigate the question. **Test:** if a new `OrchestratorEvent` variant has (a) no projector arm, (b) no replay invariant the projector relies on, and (c) no `EventPayload` mirror, it can land as a pure additive variant without an ADR addendum. If any of those is yes, write the ADR addendum.

## 2026-05-03 — Phase 23.B reduced-motion: explicit `animation: none` over the axioms.css collapse

`packages/ui/styles/axioms.css` already sets `animation-duration: 0.01ms !important; animation-iteration-count: 1 !important` under `@media (prefers-reduced-motion: reduce)`, which collapses the `--motion-pulse` keyframe to a single near-zero-duration cycle — effectively a static dot. Acceptance test T-23B-3 was specced to assert a strict `getComputedStyle(...).animation === 'none'` — and the axioms collapse does *not* satisfy that assertion (computed style still reports the `compose-dock-activity-pulse` shorthand with a 0.01ms duration, not the literal `none`).

The roadmap allowed two choices: accept the collapse and relax the test, or add an explicit `animation: none` rule on `.compose-dock-activity-row__pulse` (and `.tab-button__activity-badge`) under the same media query. Picked the explicit override so the activity-row's reduced-motion state is unambiguous in computed style (developers grepping for "is this thing actually animated" can read `animation: none` directly), and the JSDOM-level test asserts the class hook + CSS rule existence rather than digging into computed-style internals. Documented as the chosen mode; ship one, not both — chat.css carries the explicit `animation: none` rule and the test only checks for the class hook (a future browser-level test can do the full `getComputedStyle` assertion).

## 2026-05-03 — Exit curves: prefer `--ease-in-exit` for fade-outs, not `--ease-out-enter`

The friction-widget post-submit close (frc_019de6f8) is the first surface that runs an *enter* and an *exit* motion in the same frame: the filed slab fades in over the composer interior while, ~600ms later, the widget root itself fades out. Initial implementation used `--ease-out-enter` (deceleration: fast-start, slow-finish) for both — the slab fades in (correct) and the widget fades out (wrong: an exit on a deceleration curve reads as "settling into invisibility" rather than "leaving").

Switched the widget root's `transition: opacity` to `--ease-in-exit` (acceleration: slow-start, fast-finish). Now the two motions read as a *composed* gesture: the slab settles in (decel), the widget then accelerates away (accel). Same `--motion-emphasized` duration so the cadence still holds.

Codified rule: any element that is leaving the screen — modal dismiss, widget close, drawer hide, snackbar fade — uses `--ease-in-exit`. Any element entering — same surface, mounted-to-visible — uses `--ease-out-enter`. The composed `--motion-exit` token (`--motion-quick var(--ease-in-exit)`) is the right shape for snappier transient exits; `--motion-emphasized var(--ease-in-exit)` is the right shape for an affordance the user is actively reading (the friction confirmation here). Avoid `--ease-standard` for either; it's the interactive curve, not a leave/enter curve.

## 2026-05-03 — Phase 22.A — Conic-arc status circle via SVG stroke-dasharray (RoadmapStatusCircle)

The roadmap canvas needs a status glyph that animates between four partial-fill states (Backlog → InProgress at ~50% → InReview at ~85% → Done at 100% + check). The natural CSS reach is `conic-gradient(...)`, but **CSS conic-gradient does not transition between stops** — a known cross-browser issue (Chromium issue 1135291, WebKit bug 224498). Animating the gradient stops produces an instant flip, defeating the "considered liveliness" axiom #5 wanted for state changes.

The fix is to draw the partial fill as an SVG circle with an animated `stroke-dasharray`. A circle of radius `r` has circumference `2πr`; setting `stroke-dasharray: ${fill * c} ${(1 - fill) * c}` paints exactly `fill` proportion of the perimeter. Browsers transition `stroke-dasharray` smoothly, GPU-composited, at any frame rate the renderer will give us. A `transform: rotate(-90 cx cy)` puts the start of the arc at 12 o'clock so visual progression reads top-down clockwise.

Two load-bearing details: (a) **minimum render size is 14 px** (`--icon-md`). Below that the dasharray segment for InProgress (~50%) is fewer pixels than the stroke is wide, and the arc reads as a tick rather than a quarter-curve — the `data-tiny` fallback at 12 px renders a filled dot at the semantic color instead. Documented in the component manifest. (b) The circle was **forked from `WorkspaceStatusIcon` rather than extended**. WSI is a Lucide-glyph wrapper used on every workspace row; bolting an animated SVG arc onto it would risk regressing every workspace row the day the conic transition lands. Two components, one shared CSS prefix — cheaper than a unified surface.

## 2026-05-03 — Phase 22.A — Multi-claim status precedence: max-rank under all-must-ship Done gate

When more than one track claims the same roadmap node, the projection takes the **maximum** state across claiming tracks under an "all-must-ship" gate for `Done`. Order: `Backlog < Todo < InProgress < InReview < Done < Canceled`. The node projects the max of the claiming tracks' statuses, **except** Done is only emitted when *every* claiming track has a recorded `NodeShipment`. If any claim is unshipped, the node projects `InReview`.

The intuition: multi-claim is parallel work toward one node, not a race. Two teams each owning a sub-slice of "Phase 22" both need to ship before the Phase reads Done — claiming a node is a commitment, not a query. The alternative (any-shipped is enough) makes the canvas lie about what's actually merged. The alternative (first-shipped wins) makes the canvas confusing when two teams are working in parallel — we'd be telling the user "the work is done" while one team is still mid-PR.

The rule is enforced in a pure function (`derive_node_status` in `crates/designer-core/src/roadmap/derive.rs`) that takes `(claim, TrackState)` pairs + shipments + the authored fallback status. Isolating it lets us test the precedence logic in one place without setting up the full event-store machinery — every claim/lifecycle/shipment combination has a unit test in the same file. Tie-break for stable multi-claim ordering inside `node_to_claimants` is `claimed_at` ascending then `track_id` lexicographic; the data model deliberately omits `team_id` (Designer has no `Team` projection in 22.A scope and `track_id` is a UUIDv7 so the secondary sort agrees with creation time in practice).
## 2026-05-03 — Friction triage: hand-rolled `⋯` menu instead of a new Mini primitive

The friction-triage redesign (per-row collapse from 6 buttons to a `⋯` menu) needed a popover with state-conditional items. Designer has no Mini Menu / Dropdown / Popover primitive yet — every existing menu-shaped surface in the app is hand-rolled (FrictionWidget, SelectionOverlay, the QuickSwitcher modal). The staff design engineer review surfaced this and gave two options: (a) extend `IconButton` with a `menuItems` prop and a one-off popover inline in `SettingsPage.tsx`, or (b) introduce a real `Menu` primitive in `packages/ui` with manifest entry, generation-log entry, and the full archetype lift.

Picked (a). Reasons:

- **No second consumer yet.** Mini's primitive bar is "extract on the second use." A Menu primitive built for one consumer locks an API on guesswork; the right shape only emerges when a *different* surface needs one (e.g. a future tabs context menu, a workspace-row menu, an inline approval menu). Building the primitive now risks a wrong-shape API that has to be rewritten on the second consumer anyway.
- **Cost asymmetry.** A 60-line inline `FrictionRowMenu` component (trigger + popover + ESC/click-outside dismissal + auto-focus first item) ships in this PR with no archetype lift. A real Menu primitive needs the manifest entry, the generation-log entry, the design-language carve-out, and the keyboard-navigation contract (arrow keys, type-ahead, role=menu vs role=listbox). That's a separate workstream, not a side quest of the friction PR.
- **The tipping point is two consumers.** The moment a non-friction surface needs the same shape, promote `FrictionRowMenu` into `packages/ui` and have both consumers point at it. The pattern-log will read as "this PR added the second consumer; the primitive is now justified."

The hand-rolled implementation lives at `packages/app/src/layout/SettingsPage.tsx::FrictionRowMenu`. Behavior: trigger toggles open; ESC closes and restores focus to the trigger; click-outside closes; auto-focuses the first non-disabled menuitem on open. Disabled items still render (greyed) so menu height doesn't dance between rows. Items render with `role="menuitem"` inside a `role="menu"` container — the contract that a future Mini Menu primitive should preserve.

## 2026-05-03 — Friction "Addressed" state folds under Open, with an agent indicator carrying the trust signal

Three filter chips → two: dropped the explicit `Addressed` chip from `FrictionTriageSection`. Addressed rows now fold under `Open`. This is a deliberate UX bet that requires its own pattern-log entry because the obvious-looking alternative (keep all three chips) actually breaks the user's stated workflow.

The user's framing: *"I want to focus on making the reports, and I should be able to trust that the agent will aggregate them, plan out solutions, and solve them efficiently."* The chips before the redesign forced a manual mental model — "is this still on me, or did I tell the agent about it, or is it actually done?" Three states, three chips, three things to remember. The redesign collapses to two: still-on-me (Open) and done (Resolved). The agent's intermediate state ("I'm working on it") is *not* a chip — it's a marker on the row.

Why this works:

- **The trust signal lives on the row, not the chip.** When the agent picks up a report it transitions to `addressed`. The row still appears under Open, but its meta line carries `agent · <PR shortlabel>` (or `agent · working` if no PR yet). The user sees at a glance which rows are on them vs which the agent has, without having to switch chips.
- **Empty Open is context-aware.** When Open is empty *only because* every report has been picked up by the agent, the empty-state copy explicitly says "All open items are with the agent — check back when the PRs land." When Open is genuinely empty (no captures, no agent activity), the copy is the original "Press ⌘⇧F to capture something." The two empty states feel different because they *are* different.
- **The strict-equality filter would have hidden agent work.** Pre-redesign, `filter === "open"` matched only `state === "open"`. As soon as the agent ran `designer friction address`, the row vanished from the user's primary view and showed up under a chip the user wasn't looking at. That breaks the "I trust the agent" posture — the agent visibly *removed* the user's reports. The new filter matches `state ∈ {open, addressed}` so agent activity stays in the user's worklist with a marker, not exiled to a sidebar.
- **The escape hatches are intact.** Mark resolved + Reopen are still in the per-row `⋯` menu. The user can override the agent's verdict any time. The hidden affordance is `Mark addressed` itself — that transition is now exclusively the agent's job via the CLI (`designer friction address <id> --pr <URL>`), with no FE surface remaining. Justified because (a) the agent has the diff context to know what counts as "addressed," (b) a manual Mark addressed without a PR URL was always a half-step no manager wanted to take, and (c) the test of the design is "do reports really get triaged, or do they pile up?" — the path that hands the loop to an agent is the path that scales.

If the agent loop ever proves unreliable in practice (dogfood signal, not theory), the recovery is symmetrical: re-add the `Addressed` chip and the per-row Mark-addressed action. Either choice is reachable from the current shape with a small additive change — neither path is locked.

## 2026-05-04 — Compact braille spinner replaces three pulsing dots in `.thread__activity`

The `.thread__activity-dots` row (three 8 px circles, staggered scale-pulse animation, ~28 px wide before the label) was the original B7 thinking indicator. After dogfood, the user flagged it as "clunky and too big and too wide" — the row crowded the thread, and the bouncing-dot register felt off-brand for a manager-grade surface. Replacement: a single `.thread__activity-spinner` span, `1ch` wide, whose `::before` pseudo-element cycles through eight braille glyphs `⠁⠈⠐⠠⢀⡀⠄⠂` at `var(--motion-spin)` (800 ms). One dot traversing the eight positions of a braille cell — motion along a path, at one-character width.

CSS-only animation. The keyframe `content: "<glyph>"` swap is supported by every modern WebKit/Blink/Gecko build and avoids React state for the rotation. `animation-timing-function: steps(1)` snaps between frames so every paint shows one canonical glyph rather than an interpolated half-state.

Why braille and not a custom SVG spinner. Braille glyphs are Unicode text — they inherit the surface's font, color, and opacity tokens for free. An SVG spinner would need its own size/colour/motion contract and ship a second component to the manifest. Braille keeps the affordance inside the existing `.thread__activity` chrome with zero new asset surface. The "developer-tool" risk (Conductor uses similar glyphs for activity in its sidebar) was raised in design review and discarded — at caption density, the rotation reads as "indicator," not "CLI artifact."

Glyph order and direction. The eight glyphs walk top-right → top-left → bottom-left → bottom-right → top-right (clockwise traversal of the 8 dot positions of a braille cell). Picked over counter-clockwise empirically — the eye tracks the dot left-to-right at the start of each cycle, matching English reading direction.

Reduced-motion fallback uses `…` (U+2026 horizontal ellipsis), not the all-dots braille `⠿`. First pass shipped `⠿`; UX review caught that against the rotation frames (each a single dot in the same cell), `⠿` reads as a frozen frame — an indicator stuck mid-rotation rather than an indicator that's deliberately paused for accessibility. Horizontal ellipsis is an unambiguous "in progress" affordance regardless of motion state and renders at 1ch in every monospace family. The trade-off: ellipsis is not visually consistent with the rotation frames; we accept that because the reduced-motion contract is "show that *something* is happening, not a slowed version of the motion."

Cross-surface durations stay separate by intent. `--motion-blink: 700ms` (streaming cursor) signals "text is arriving"; `--motion-spin: 800ms` (thinking spinner) signals "agent is computing." Both ~1 s but semantically different — one is per-character cadence, the other is per-rotation. `--motion-pulse: 1200ms` (compose-dock activity row) is the third register: workspace-level "something is happening" at calmer cadence. Three durations for three indicators is intentional; collapsing to one would erase the speed-of-feedback differential.
...
## 2026-05-12 — `--motion-fast` → `--motion-exit`; monochrome activity pulse

Phase 24 §5.2 specified two motion + color tokens that didn't fit the project's existing palette:

**1. `--motion-fast` for the chip's exit fade.** No such token exists in `packages/ui/styles/tokens.css`. The motion tier list is `--motion-quick: 120ms` / `--motion-standard: 250ms` / `--motion-pulse: 1200ms`, plus the composed `--motion-exit: var(--motion-quick) var(--ease-in-exit)` and `--motion-enter: var(--motion-standard) var(--ease-out-enter)`. The first cut of Step 10 used `transition: opacity var(--motion-quick) ease-out;` — disassembling the composed `--motion-exit` token into duration + raw easing literal. Staff-review caught it: the design-language pairs `--ease-in-exit` with exits, not `ease-out`, and the composed `--motion-exit` token *is* the contract. The fix is `transition: opacity var(--motion-exit);` — the compound captures both pieces. The JS unmount-after-fade timeout (`CHIP_EXIT_MS = 120` in `ComposeDockActivityRow.tsx`) mirrors the duration component of `--motion-exit` (`--motion-quick` = 120 ms), with a comment that ties the two together.

**Generalization to non-compound cases.** Sometimes a surface genuinely needs a different duration than the composed token's pair (e.g., SendMenu uses `--motion-quick` 120 ms for a hover-revealed menu — snappier than `--motion-enter`'s 250 ms, which would feel sluggish for a hover popover). In that case, **still use the project's named easing tokens** (`--ease-out-enter` / `--ease-in-exit` / `--ease-standard`), never raw `ease-out` / `ease-in` / `linear` literals. The composed `--motion-enter` / `--motion-exit` tokens are the default; the named easings under them are the escape hatch for legitimately-different durations. The same Step 10 review caught a sibling violation in `chat.css:249` from PR #124 — `animation: send-menu-in var(--motion-quick) ease-out;` was using the raw `ease-out` literal. Fixed inline in the same PR to `var(--motion-quick) var(--ease-out-enter)` — keeps the snap, uses the named easing.

**Rule:** don't introduce ad-hoc motion tokens to mirror spec aspirational naming. Prefer the composed motion tokens (`--motion-exit`, `--motion-enter`) when the surface's duration matches the standard pair. When the duration genuinely differs, still use the named easing tokens (`--ease-out-enter` / `--ease-in-exit` / `--ease-standard`) — never raw `ease-out` / `ease-in` literals. The named easings encode the project's choice about motion personality; raw CSS easings sidestep that choice.
...
## 2026-05-15 — Scroll-stickiness on a streaming surface: ResizeObserver on `firstElementChild` + microtask programmatic-scroll guard

Phase 24H W1a (PR #144) closed spec Q4 by porting the legacy B6 scroll-stickiness pattern onto the chat-v2 surface (`.thread--phase24`). Two implementation choices warrant a pattern-log entry so the next person solving a similar problem doesn't re-derive them.

**1. ResizeObserver on the scroll container's `firstElementChild`, not the container itself.** A streaming chat surface extends the height of an open text block on each `AgentContentBlockDelta` without changing any value the parent's `useLayoutEffect` deps catch — `artifacts.length` is stable, `hasStarted` is stable, `sending` is stable, and the per-tab chat-thread state lives inside `ChatStreamRenderer`, not `WorkspaceThread`. The scroll container's own size doesn't change (it's `overflow-y: auto` flex-1); only its content grows. `ResizeObserver` observing the *container* doesn't fire on content growth — RO entries report the observed element's content/border rect, not its scrollable content. Observing the `firstElementChild` (the renderer's root inside the scroll container) catches the content-growth contract directly.

This pattern works for both branches (legacy artifact-list and chat-v2 `.thread__stream`) without a flag check — `el.firstElementChild` is whichever content node React rendered. The effect's deps include the chat-thread store's `bootReplaying` flag so the observer re-binds when the chat-v2 surface flips from `<LoadingState/>` to the populated `<div className="thread__stream">`; without that dep, the observer would hold a stale binding to the unmounted LoadingState and miss the populated-thread arrival.

**Rule.** When a streaming surface's content grows inside a fixed-size scroll container, observe the *content child*, not the container. When the content node identity changes (a state transition swaps a different child in), the observer's effect deps must include the signal that drives the swap — otherwise the observer holds a binding to a detached element. ResizeObserver's first callback after `observe()` fires immediately with the current size, which doubles as a "we just re-bound; pin if sticky" trigger — no separate initial-pin code needed.

**2. `queueMicrotask`, not `setTimeout(0)`, for the programmatic-scroll guard.** The guard prevents auto-pin scrolls (where the implementation assigns `scrollTop`) from being interpreted as user scrolls by the natural `scroll` event the browser fires afterward. The contract is: set the guard true synchronously before the assignment; clear it after any synchronously-dispatched scroll events have fired; do not swallow real user scroll events that arrive later in the same tick.

`queueMicrotask` runs after the current synchronous block finishes but before the next event loop tick — tight enough that real user scrolls on the *next* tick land with the guard cleared. `setTimeout(0)` would also work but adds a task-queue hop (4 ms minimum in nested timers per the HTML spec), widening the window in which a user scroll could be incorrectly swallowed. `Promise.resolve().then(...)` is equivalent to `queueMicrotask` here; the explicit name reads better.

**Rule.** Use `queueMicrotask` for guards that need to span "the current synchronous block plus any events it dispatches." Reserve `setTimeout(0)` for guards that genuinely need to wait until the next task boundary (rare). The tighter guard keeps user-event responsiveness intact while still absorbing the synthesized scroll-event race the guard exists to suppress.

The PR's render-altitude test (`chat-scroll.test.tsx` T-24H-W1a-5) pins this contract: fire a programmatic scroll inside the guard window → synthesized scroll event is absorbed (no pill appears); `await Promise.resolve()` to drain the microtask → next user scroll re-arms the pill. The microtask-vs-task distinction is what makes the test deterministic.
