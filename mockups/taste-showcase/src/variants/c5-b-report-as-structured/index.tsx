...
// c5-b — Report as structured artifact (earn the box).
// Tests the hypothesis: a report deserves a container IF it carries real
// affordances — link to the PR, list of changed files, expandable test
// output. The container becomes honest; it no longer claims structure that
// isn't there. Inherits c4-a's base; only the Report differs.

export default function C5BReportAsStructured() {
  const groups = groupArtifacts(thread);
  return (
    <div className="chat c4 c5-b">
      <header className="chat__head">
        <h1 className="chat__head-title">{workspaceTitle}</h1>
        <p className="chat__head-sub">c5-b — report as structured artifact</p>
...
function Report({ a }: { a: Extract<Artifact, { kind: "report" }> }) {
  // c5-b: structured artifact. The container carries:
  //   - classification chip + title (what shipped)
  //   - body (what was done, in one sentence)
  //   - PR link (where to review)
  //   - files changed list (what was touched)
  //   - expandable test output (proof of correctness)
  // These are real affordances the user can interact with. The box now
  // honestly represents structure.
  const [logOpen, setLogOpen] = useState(false);
  const filesChanged = ["packages/app/src/tabs/WorkspaceThread.tsx", "packages/app/src/tabs/WorkspaceThread.test.tsx"];
  return (
    <div className="c5-b__report">
      <div className="c5-b__report-head">
        {a.classification && (
          <span className="c5-b__chip" data-kind={a.classification}>
            {a.classification === "fix" ? "Fix" : a.classification === "feature" ? "Feature" : "Improvement"}
          </span>
        )}
        <span className="c5-b__report-title">{a.title}</span>
        <a className="c5-b__report-pr" href="#" onClick={(e) => e.preventDefault()}>
          PR #142 ↗
        </a>
      </div>
      <p className="c5-b__report-body">{a.body}</p>
      <div className="c5-b__report-meta">
        <div className="c5-b__report-files">
          <span className="c5-b__report-meta-label">Files</span>
          <ul>
            {filesChanged.map((f) => (
              <li key={f}>
        ...
        <div className="c5-b__report-tests">
          <span className="c5-b__report-meta-label">Tests</span>
          <button
            className="c5-b__report-tests-toggle"
            onClick={() => setLogOpen((v) => !v)}
            onMouseUp={(e) => e.currentTarget.blur()}
            aria-expanded={logOpen}
          >
            <span className="c5-b__report-tests-result">110 passed (+3 new)</span>
            <span className="c5-b__report-tests-action">{logOpen ? "Hide log" : "Show log"}</span>
          </button>
          {logOpen && (
            <pre className="c5-b__report-log">{`vitest run

  ✓ packages/app/src/tabs/WorkspaceThread.test.tsx (3)
    ✓ stickiness pins to bottom when at edge
    ✓ stickiness preserves position when scrolled up
    ✓ jump-to-latest pill renders on new content arrival
