...
// c5-a — Report as prose (drop the box).
// Tests the hypothesis: a "report" with only title + body doesn't earn a container.
// It's just the agent telling you what it shipped — that's prose. The box was
// claiming structure that the content didn't actually have.
//
// Inherits c4-a's base entirely (chat baseline, diff-box for code-change, no
// invented glyphs, tool-run with chevron, equal bubble padding). Only the
// Report component differs: rendered as agent prose with an h3 + paragraphs,
// matching how the agent's other voice messages render.

export default function C5AReportAsProse() {
  const groups = groupArtifacts(thread);
  return (
    <div className="chat c4 c5-a">
      <header className="chat__head">
        <h1 className="chat__head-title">{workspaceTitle}</h1>
        <p className="chat__head-sub">c5-a — report as prose</p>
...
function Report({ a }: { a: Extract<Artifact, { kind: "report" }> }) {
  // c5-a: render as prose, no container. The agent reporting what shipped is
  // a voice statement — same register as the agent's other messages. Title
  // becomes an h3, body becomes paragraphs. Classification is dropped entirely;
  // the verb in the title ("shipped") carries the meaning.
  return (
    <div className="chat__prose c4__prose c5-a__report">
      <h3>{a.title}</h3>
      <p>{a.body}</p>
    </div>
  );
