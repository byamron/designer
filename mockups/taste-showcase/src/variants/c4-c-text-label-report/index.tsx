...
// c4-c — same base as c4-a / c4-b. Differs only in Report:
// clean bordered container PLUS a small uppercase text label
// ("FIX", "FEATURE", "IMPROVEMENT") above the title.
// No chip, no rail. Tests whether the classification adds value as plain typography.

export default function C4CTextLabelReport() {
  const groups = groupArtifacts(thread);
  return (
    <div className="chat c4 c4-c">
      <header className="chat__head">
        <h1 className="chat__head-title">{workspaceTitle}</h1>
        <p className="chat__head-sub">c4-c — text-label report</p>
...
function Report({ a }: { a: Extract<Artifact, { kind: "report" }> }) {
  // c4-c variant: clean bordered container, small uppercase text label above title.
  return (
    <div className="c4-c__report">
      {a.classification && (
        <span className="c4-c__report-label">{a.classification}</span>
      )}
      <p className="c4-c__report-title">{a.title}</p>
      <p className="c4-c__report-body">{a.body}</p>
    </div>
  );
}
