...
6. **NEVER file review follow-ups only in the PR body.** Every FOLLOW-UP produced by a structured review pass (`/staff-perspective-review`, `/security-review`, ad-hoc multi-perspective review, etc.) MUST land in `core-docs/roadmap.md` (active section if it gates a current Build/Harden phase) or `core-docs/parking-lot.md` (with a friction-driven primary trigger + time-based fallback per ADR 0009) before the review pass closes. The PR body MAY cross-reference these entries for the human reviewer's convenience, but the roadmap or parking-lot is the canonical home — closed PR bodies are not searchable, and "Follow-ups" sections rot the moment the PR merges.
