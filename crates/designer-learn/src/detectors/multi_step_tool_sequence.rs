...
//!
//! - **Session.** Each user [`EventPayload::MessagePosted`] opens a new
//!   session. Tool-use artifacts that arrive before the workspace's
//!   first user message belong to no session and are discarded — there
//!   is no [`Anchor::MessageSpan`] target to attach them to.
...
//! - **Sequence.** A length-[`MultiStepToolSequenceDetector::SEQUENCE_LEN`]
//!   sliding window over a run. A run of length R yields
//!   `R - SEQUENCE_LEN + 1` sequences when `R >= SEQUENCE_LEN`, else
//!   zero.
//!
...
//!   address.
//! - `evidence` carries up to
//!   [`MultiStepToolSequenceDetector::MAX_ANCHORS_PER_KIND`]
//!   [`Anchor::MessageSpan`] anchors (one per matched session, taken in
//!   first-seen order) plus the same cap on [`Anchor::ToolCall`]
//!   anchors (one per identified sequence run). The summary keeps the
//!   uncapped session and occurrence counts.
...
impl MultiStepToolSequenceDetector {
    ...
    /// Per-kind anchor cap. A finding emits up to this many MessageSpan
    /// anchors and up to this many ToolCall anchors, matching the
    /// approval-detector convention. The summary keeps the uncapped
    /// occurrence + session counts; anchors are spot-check pointers,
    /// not an exhaustive log.
    pub const MAX_ANCHORS_PER_KIND: usize = 5;
...
/// should not participate in sequence aggregation. Borrows from the
/// input title for the `Used <Tool>` case, so the caller decides when
/// to allocate.
fn extract_tool_name(title: &str) -> Option<&str> {
...
/// One tool-use entry inside a run.
#[derive(Debug, Clone)]
struct RunEntry {
    tool: String,
    event_id: String,
...
/// Anchor data captured at the first user message of a session. The
/// `quote` is stored verbatim so a stale `char_range` could be
/// re-located by string search after edits (per `Anchor::MessageSpan`
/// docstring).
#[derive(Debug, Clone)]
...
/// Aggregation per tuple identity. Sessions are stored as a `BTreeSet`
/// of session indices (cheap distinct-count, deterministic iteration);
/// per-session run anchors are stored as a `BTreeMap` keyed by the same
/// index so finding evidence groups by session in arrival order.
...
fn detect(input: &SessionAnalysisInput, config: &DetectorConfig) -> Vec<Finding> {
    ...
    // Tools that arrive before the first user message have nowhere to
    // anchor; `current_session` stays `None` until the first
    // `MessagePosted{ author: User }` and pre-message tools are
    // discarded by the implicit `flush_run` no-op below.
    let mut sessions: Vec<SessionAnchor> = Vec::new();
...
/// `tuples`. `session` is the session that owns this run; `None`
/// (pre-first-user-message) discards the run entirely.
fn flush_run(
    run: &mut Vec<RunEntry>,
    session: Option<usize>,
    tuples: &mut HashMap<Vec<String>, TupleEvidence>,
) {
    let Some(session_idx) = session else {
        run.clear();
        return;
    };
...
fn build_finding(
    ...
    let chain_label = tuple.join(" \u{2192} ");

    // Anchors are spot-check pointers, not an exhaustive log: cap each
    // kind at MAX_ANCHORS_PER_KIND. The summary keeps the uncapped
    // counts so the user knows what the cap hides.
    let cap = MultiStepToolSequenceDetector::MAX_ANCHORS_PER_KIND;
    let mut message_anchors: Vec<Anchor> = Vec::new();
    let mut tool_anchors: Vec<Anchor> = Vec::new();
    let mut digest_keys: Vec<String> = vec![format!("tuple:{}", tuple.join(">"))];

    for session_idx in &evidence.sessions {
        if let Some(anchor) = sessions.get(*session_idx) {
            if message_anchors.len() < cap {
                message_anchors.push(Anchor::MessageSpan {
        ...
        if let Some(run_ids) = evidence.runs_by_session.get(session_idx) {
            for event_id in run_ids {
                if tool_anchors.len() < cap {
                    tool_anchors.push(Anchor::ToolCall {
                        event_id: event_id.clone(),
                        tool_name: chain_label.clone(),
                    });
                }
    ...
    let mut anchors = message_anchors;
    anchors.append(&mut tool_anchors);
    ...
    Finding {
        ...
        confidence,
        summary: build_summary(&chain_label, evidence.total_count, session_count),
        evidence: anchors,
...
fn build_summary(chain: &str, total: u32, sessions: u32) -> String {
    let raw = format!("Tool sequence {chain} repeated {total}\u{00d7} across {sessions} sessions");
    truncate_with_ellipsis(raw, MultiStepToolSequenceDetector::SUMMARY_BUDGET)
}
...
fn quote_snippet(body: &str) -> String {
    truncate_with_ellipsis(
        body.to_string(),
        MultiStepToolSequenceDetector::QUOTE_BUDGET,
    )
}

/// Char-safe truncation with a trailing ellipsis. Returns `s` verbatim
/// when it already fits; otherwise the result has exactly `budget`
/// chars and ends in `…`.
fn truncate_with_ellipsis(s: String, budget: usize) -> String {
...
mod tests {
    ...
    fn extract_tool_name_handles_known_verbs() {
        assert_eq!(extract_tool_name("Read CLAUDE.md"), Some("Read"));
        assert_eq!(extract_tool_name("Wrote x.txt"), Some("Write"));
        assert_eq!(extract_tool_name("Edited foo.rs"), Some("Edit"));
        assert_eq!(extract_tool_name("Searched files"), Some("Search"));
        assert_eq!(extract_tool_name("Searched for \"foo\""), Some("Search"));
        assert_eq!(extract_tool_name("Ran git status"), Some("Bash"));
        assert_eq!(extract_tool_name("Used WebFetch"), Some("WebFetch"));
    ...
    fn build_summary_stays_inside_budget() {
        let s = build_summary("Read \u{2192} Edit \u{2192} Bash", 4, 3);
    ...
    fn truncate_with_ellipsis_preserves_short_strings() {
        assert_eq!(truncate_with_ellipsis("hi".to_string(), 10), "hi");
        assert_eq!(truncate_with_ellipsis("abcdef".to_string(), 6), "abcdef");
        let truncated = truncate_with_ellipsis("abcdef".to_string(), 4);
        assert_eq!(truncated, "abc\u{2026}");
    ...
    #[tokio::test]
    async fn pre_message_tool_runs_are_discarded() {
        // Tools that arrive before any user MessagePosted have no
        // session anchor, so they must not contribute to tuple
        // aggregation — even when the same tuple later repeats inside
        // real sessions, the pre-message instance is invisible.
        let ws = WorkspaceId::new();
        let mut events = Vec::new();
        let mut seq = 0u64;
        // Pre-message run on (Read, Edit, Bash). No session yet.
        for title in ["Read CLAUDE.md", "Edited foo.rs", "Ran cargo test"] {
            seq += 1;
            events.push(tool_artifact(seq, ws, title));
        }
        // Two real sessions with the same tuple — only 2 distinct
        // sessions count under the new model.
        for i in 0..2 {
            seq += 1;
            events.push(user_message(seq, ws, &format!("session {i}")));
            for title in ["Read CLAUDE.md", "Edited foo.rs", "Ran cargo test"] {
        ...
        assert!(
            findings.is_empty(),
            "pre-message run inflated the session count: {findings:?}"
        );
    }

    #[tokio::test]
    async fn anchors_capped_at_max_per_kind() {
        // 12 sessions, each running (Read, Edit, Bash). Far above the
        // anchor cap; the summary should still show 12 sessions.
        let ws = WorkspaceId::new();
        let mut events = Vec::new();
        let mut seq = 0u64;
        let session_count = 12u32;
        ...
        let msg_count = f
            .evidence
            .iter()
            .filter(|a| matches!(a, Anchor::MessageSpan { .. }))
            .count();
        let tool_count = f
            .evidence
            .iter()
            .filter(|a| matches!(a, Anchor::ToolCall { .. }))
            .count();
        assert_eq!(msg_count, cap, "MessageSpan anchors not capped");
        assert_eq!(tool_count, cap, "ToolCall anchors not capped");
        assert!(
            f.summary.contains(&format!("{session_count} sessions")),
